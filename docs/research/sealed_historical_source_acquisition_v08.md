# Sealed historical source recovery v0.8

## Status and purpose

Run `33530672018` dispatched v0.7 once and failed closed in validation at
...
gates before it passed. The failed step invoked `gh api` four times inside
opaque shell `test` expressions and retained none of the observed field values,
so the exact comparison that returned nonzero cannot be recovered from the
sanitized log. Consume, acquire and freeze were skipped. No consumption tag,
run artifact or provider request was created, and v0.7 may not be rerun.

Independent post-run inspection shows GitHub artifact `9806541315` still has
the exact expected name, digest, parent run, size and unexpired status. v0.8
therefore preserves fail-closed semantics while replacing the unobservable
four-request shell gate with one retained JSON response and one deterministic,
...
v0.8 is an additive, one-shot recovery child. It leaves v0.7 and every consumed
v0.6 file reproducible, reuses the exact retained source, preserves the v0.7
downstream identity repair, and changes only the provider-free artifact-metadata
preflight. Scanner profiles, thresholds, Micro-v0.1, news rules, float
...
regular JSON file. The v0.8 validator rejects duplicate keys, non-finite values,
...
failure, never an unbounded response value. The artifact is still downloaded,
rehash-verified and deeply replayed before authorization consumption.

## Downstream compatibility repair

The v0.8 compatibility scope accepts exactly the upstream identity contract:
...
Before v0.8 can be consumed, a provider-free preflight deep-loads the complete
...
is available to v0.8. The child transport guard admits only
...
1. Validate the frozen v0.8 authorization, registration and exact v0.7 safe
...
3. Create the v0.8 consumption marker and atomically create its protected Git
   tag.
