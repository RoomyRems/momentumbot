...
> **Source correction (2026-08-17):** ARTL's old Micro label was misattributed from an earlier stock discussion. The historical run remains preserved, but the current comparison excludes ARTL: baseline and v0.2e both score **8/10**, while v0.2e raises activity from 11 plans/7 fills to 35 plans/12 fills. See `docs/research/artl_source_label_correction.md`.
...
The historical run did not reacquire market data. It downloaded five exact label-blind runtime bundles from authoritative Micro v0.1 workflow run `31925087895`, verified each ZIP digest, and required the current parent replay to reproduce every frozen v0.1 runtime-core field before applying the ablation. The ARTL runtime remains valid but its later label comparison is now excluded. Future workflow executions use only the four valid scored cases.
...
Across the four valid cases, aggregate plans increase from 11 to 35 (+24) and modeled fills from 7 to 12 (+5). Both baseline and ablation score 8/10; no case moves its first price or pullback ordinal.
...
This closes only this base definition; it does not prove that the frozen five-bar impulse translation is uniquely correct. The four-case scored seed should not be mined for another geometry variant. A further Micro change requires a separately precommitted, representative validation design rather than another rule chosen from these labels.
