...
`account-session-snapshot-capture-v0.1` is the read-only input adapter for that composer. It binds separately credentialed $30,000 main and $2,000 small Alpaca paper fixtures, requires active USD accounts with no open positions/orders or blocked flags, and captures by 7:00 a.m. New York time on each registered session. Raw provider account IDs are replaced by stable SHA-256 pseudonyms; credentials and nonrequired provider fields are never stored. Source projections, account snapshots and the bundle manifest are independently hash-bound. The default-branch scheduler explicitly checks out the frozen research branch and records both SHAs. Validation-only output has no session authority, and neither mode can submit an order.
