"""Independent terminal-continuation authority, chronology and accounting checks.

Frozen arithmetic is copied without modification. The isolated changes admit
third and later terminal orders only with the separately checked authority chain.
No producer import or second execution fill simulator is used.
"""
import argparse
...
from verify_sealed_historical_account_residual_exit_v01 import (
    WAITING_ID, TAIL, MINUTE, References, OriginalEvidence, evidence, eligible, terminal_reason,
    account_state, priority, public_collections, ready, exact_money, checked_tree,
    number, BOUND, boundaries, source_for_events, seal, ns,
)
from verify_sealed_historical_account_residual_exit_status_v01 import cancellation_status
...
RESIDUAL_ID = "sealed-historical-account-residual-exit-v0.1"
...
def verify_residual(runtime, resolve):
    ...
    for event in snapshot.get("exit_residual_events", []):
        ...
        require(event["contract_id"] == RESIDUAL_ID and event["opportunity_id"] == entry["opportunity_id"]
        ...
        terminal = [e for e in orders.get(entry["opportunity_id"], []) if e["reason"] != "first_target"][:2]
    ...
    for oid, entry in by_oid.items():
        terminal = [e for e in orders.get(oid, []) if e["reason"] != "first_target"][:2]
        if terminal:
...
def verify_continuation(runtime, resolve):
    snapshot = runtime["reconciliation_snapshot"]
    counts = {"continuation_ready": 0, "continuation_proposals": 0,
        "continuation_submissions": 0, "continuation_expiries": 0}
    if snapshot is None:
        return counts
    entries = {j["execution_evidence"]["content_sha256"]: j["execution_evidence"]
        for j in snapshot["journal"] if j["fee_application"]["trade"]["side"] == "buy"}
    sells = [j["execution_evidence"] for j in snapshot["journal"]
        if j["fee_application"]["trade"]["side"] == "sell"]
    orders = {eid: [e for e in runtime["events"] if e["event_type"] == "sell_submitted"
        and e["opportunity_id"] == entry["opportunity_id"] and e["reason"] != "first_target"]
        for eid, entry in entries.items()}
    roots = {e["entry_content_sha256"]: e for e in snapshot.get("exit_residual_events", [])
        if e["event_type"] == "residual_budget_exhausted"}
    def sold(entry, at):
        return [s for s in sells if s["entry_fill_id"] == entry["fill_id"] and s["fill_time_ns"] <= at]
    def quantity(entry, at):
        return entry["quantity"] - sum(s["quantity"] for s in sold(entry, at))
    chains, states, submitted = {}, {}, set()
    for event in snapshot.get("exit_continuation_events", []):
        checked(event)
        eid, at, kind = event["entry_content_sha256"], event["timestamp_ns"], event["event_type"]
        require(eid in entries and eid in roots, "continuation lacks confirmed entry or parent exhaustion")
        entry, terminal = entries[eid], orders[eid]
        data = resolve(entry["opportunity_id"])
        window = data["window"]
        chain = chains.setdefault(eid, [])
        require(event["contract_id"] == ID and event["opportunity_id"] == entry["opportunity_id"]
            and event["sequence"] == len(chain)
            and event["previous_event_sha256"] == (chain[-1]["content_sha256"] if chain else None)
            and (not chain or at >= chain[-1]["timestamp_ns"]), "continuation event chain differs")
        chain.append(event)
        require(entry["fill_time_ns"] < at < window["end_ns"], "continuation outside original window")
        context = checked(event["context"])
        require(set(context) == {"parent_residual_exhaustion_content_sha256",
            "prior_submission_intent_content_sha256", "prior_order_id", "cancel_acknowledgement",
            "terminal_attempt_number", "content_sha256"}, "continuation context scope differs")
        number = context["terminal_attempt_number"]
        require(type(number) is int and number >= 3
            and context["parent_residual_exhaustion_content_sha256"] == roots[eid]["content_sha256"],
            "continuation root or attempt differs")
        remaining = quantity(entry, at - int(kind in {"continuation_proposed", "continuation_submitted"}))
        target = entry["quantity"] // 2
        target_filled = sum(s["quantity"] for s in sold(entry, at - 1) if s["reason"] == "first_target")
        stop = float(entry["fill_price"]) if target and target_filled == target else entry["initial_stop_price"]
        require(event["remaining_quantity"] == remaining > 0 and event["active_stop_price"] == stop,
            "continuation confirmed shares or stop differs")
        require(event["continuation_attempts"] == number - (2 if kind == "continuation_submitted" else 3),
            "continuation attempt consumption differs")
        key = (eid, number)
        if kind == "continuation_ready":
            require(key not in states and len(terminal) >= number - 1, "duplicate or invented continuation authority")
            prior = terminal[number - 2]["order"]
            ack = checked(context["cancel_acknowledgement"])
            filled = sum(s["quantity"] for s in sold(entry, prior["cancel_ack_ts_ns"]) if s["order_id"] == prior["order_id"])
            require(context["prior_order_id"] == prior["order_id"]
                == "exit-" + context["prior_submission_intent_content_sha256"]
                and ack["event_type"] == "sell_cancel_acknowledged" and ack["order_id"] == prior["order_id"]
                and at == ack["timestamp_ns"] == prior["cancel_ack_ts_ns"]
                and ack["cancelled_quantity"] == prior["quantity"] - filled > 0
                and ack["execution_status"] == cancellation_status(entry, prior, filled, data),
                "continuation cancellation witness differs")
            require(event["clock_phase"] in (0, 1, 2)
                and (event["known_at_ns"] > at or (event["known_at_ns"] == at and event["clock_phase"] == 2)),
                "continuation precedes acknowledged feedback")
            if number == 3:
                require(ack == roots[eid]["cancel_acknowledgement"], "continuation changed parent cancellation")
                previous = next(e for e in snapshot["exit_residual_events"]
                    if e["entry_content_sha256"] == eid and e["event_type"] == "residual_submitted")
            else:
                require((eid, number - 1) in states and states[(eid, number - 1)]["submission"] is not None,
                    "continuation skipped prior actual submission")
                previous = states[(eid, number - 1)]["submission"]
            require(previous["intent"]["content_sha256"] == context["prior_submission_intent_content_sha256"]
                and previous["order"] == prior, "continuation prior actual intent differs")
            states[key] = {"context": context, "entry": entry, "data": data,
                "proposal": None, "submission": None, "expiry": None}
            counts["continuation_ready"] += 1
            continue
        require(key in states, "continuation event has no acknowledged authority")
        state = states[key]
        require(context == state["context"] and state["submission"] is None and state["expiry"] is None,
            "continuation changed or reused authority")
        if kind == "continuation_proposed":
            require(state["proposal"] is None, "continuation reproposed outside waiting")
            state["proposal"] = event
            counts["continuation_proposals"] += 1
        elif kind == "continuation_submitted":
            require(state["proposal"] is not None and len(terminal) >= number
                and terminal[number - 1]["order"] == event["order"], "continuation submission differs")
            intent, order = checked(event["intent"]), event["order"]
            require(intent["terminal_continuation"] == context and intent["entry_content_sha256"] == eid
                and intent["quantity"] == remaining == order["quantity"]
                and intent["decision_ts_ns"] == at == order["decision_ts_ns"]
                and terminal[number - 1]["reason"] == intent["reason"] != "first_target"
                and order["order_id"] == "exit-" + intent["content_sha256"]
                and context["cancel_acknowledgement"]["timestamp_ns"] < at
                and at + TAIL < window["end_ns"]
                and References(data["tape"]).at(window, at) is not None, "continuation order signal or clock differs")
            require(order["order_id"] not in submitted, "duplicate continuation submission")
            submitted.add(order["order_id"])
            state["submission"] = event
            counts["continuation_submissions"] += 1
        else:
            require(kind == "continuation_expired" and at + TAIL >= window["end_ns"]
                and event["original_window_end_ns"] == window["end_ns"], "continuation expiry differs")
            state["expiry"] = event
            counts["continuation_expiries"] += 1
    for (eid, number), state in states.items():
        entry, data, context = state["entry"], state["data"], state["context"]
        first = next((r for r in data["trades"] if context["cancel_acknowledgement"]["timestamp_ns"] < r["timestamp_ns"]
            and r["timestamp_ns"] + TAIL < data["window"]["end_ns"] and eligible(r["record"])), None)
        proposal, actual = state["proposal"], state["submission"]
        if runtime["complete_streams_verified"] and actual is None:
            require(state["expiry"] is not None, "unsubmitted continuation expiry omitted")
        if proposal:
            require(first is not None, "continuation invented eligible print")
            reason = terminal_reason(entry, data, runtime, first)
            red = next(({"signal_ts_ns": b["timestamp_ns"] + MINUTE, "evidence": evidence(b, "raw_sip_1m_bars")}
                for b in data["bars"] if entry["fill_time_ns"] < b["timestamp_ns"] + MINUTE
                <= min(first["timestamp_ns"], data["window"]["signal_end_ns"]) and b["record"]["c"] < b["record"]["o"]), None)
            expected = seal({**{k: entry[k] for k in ("opportunity_id", "path_id", "session_id", "scenario_id")},
                "entry_content_sha256": eid, "decision_ts_ns": first["timestamp_ns"], "reason": reason,
                "quantity": proposal["remaining_quantity"], "trade_evidence": evidence(first, "sip_transactions"),
                "red_signal": red if reason == "first_red_candle" else None, "terminal_continuation": context})
            require(proposal["timestamp_ns"] == first["timestamp_ns"] and proposal["intent"] == expected
                and reason is not None, "not first eligible continuation signal or frozen priority")
            if actual:
                if actual["intent"] != expected:
                    require(any(w["event_type"] == "wait_submitted" and w["submission_intent"] == actual["intent"]
                        and w["original_signal_content_sha256"] == expected["content_sha256"]
                        for w in snapshot.get("exit_wait_events", [])), "continuation delayed order lacks wait evidence")
            else:
                require(runtime["failure"] is not None or any(w["event_type"] == "wait_expired"
                    and w["original_signal_content_sha256"] == expected["content_sha256"]
                    for w in snapshot.get("exit_wait_events", [])), "continuation proposal omitted submission or wait")
        else:
            require(actual is None and (runtime["failure"] is not None or (first is None and state["expiry"] is not None)),
                "eligible continuation proposal omitted")
    for eid, terminal in orders.items():
        for index, row in enumerate(terminal):
            if index >= 2:
                require(row["order"]["order_id"] in submitted, "extra terminal order lacks continuation authority")
            if index >= 1 and runtime["complete_streams_verified"] and quantity(entries[eid], row["order"]["cancel_ack_ts_ns"]) > 0:
                require((eid, index + 2) in states, "acknowledged continuation authority omitted")
    return counts


def verify_parent_prefix(before, after):
    """Preserve completed sessions and exact events through the second cancellation."""
    require(before["path_id"] == after["path_id"], "parent path changed")
    for old_pair, new_pair in zip(before["sessions"], after["sessions"]):
        ...
        audit = [] if snapshot is None else snapshot.get("exit_continuation_events", [])
        if not audit:
            require(old_pair == new_pair, "pre-continuation parent session changed")
            continue
        first = audit[0]
        require(first["event_type"] == "continuation_ready", "first continuation lacks acknowledgement")
        ack = first["context"]["cancel_acknowledgement"]["timestamp_ns"]
        require([e for e in current["events"] if e["at_ns"] <= ack]
            == [e for e in prior["events"] if e["at_ns"] <= ack], "pre-continuation parent events changed")
        old_snapshot = prior["reconciliation_snapshot"]
        old_engine = old_snapshot["management"]
        require(old_engine is not None and old_engine["remaining_quantity"] == first["remaining_quantity"]
            ...
            "parent two-order acknowledged remainder changed")
        require([e for e in snapshot["exit_residual_events"] if e["timestamp_ns"] <= ack]
            == old_snapshot["exit_residual_events"], "parent residual authority prefix changed")
        require([e for e in snapshot["journal"] if e["execution_evidence"]["fill_time_ns"] <= ack]
            == old_snapshot["journal"], "parent confirmed fill prefix changed")
        return
    require(before == after, "no-continuation path differs from parent")
...
def verify_episode(wait, runtime, finish):
    ...
    if "terminal_continuation" in origin:
        identity["terminal_continuation"] = deepcopy(origin["terminal_continuation"])
    def red_at(at):
        for b in data["bars"]:
    ...
    reason = (terminal_reason(entry, data, runtime, row) if ("residual_exit" in origin or "terminal_continuation" in origin) else reason_at(row, None))
    if reason is None:
    ...
    for index, event in enumerate(runtime["events"]):
        ...
        if kind == "opportunity_disposition":
            ...
            if event["reason"] == "first_target":
                ...
                require(order["quantity"] == quantity, "terminal quantity differs")
...
def verify(root, runtime_root, binding_root, parent_runtime_root, expected_runtime_sha, paths, expected_registration_sha):
    ...
    require(registration["content_sha256"] == expected_registration_sha, "external registration commitment differs")
        ...
        with localcontext() as context:
            ...
            for program, before, after in zip(programs, old["paths"], result["paths"]):
                ...
                for pair in after["sessions"]:
                    ...
                    for key, count in verify_continuation(runtime, sources).items():
                        totals[key] = totals.get(key, 0) + count
    ...
    return parent.seal({"contract_id": ID, "verification_passed": True,
        ...
        "paths": counts, "totals": totals, "parent_chronology_checks_preserved_except_registered_terminal_continuation": True,
        "original_waiting_signals_and_first_eligible_fresh_prints_verified": True,
        "pre_continuation_parent_event_prefix_unchanged": True,
        "residual_and_continuation_cancel_signal_quantity_and_earliest_print_verified": True, "independent_fill_simulation": False,
