...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run a separately authorized one-shot INTJ instrument-event "
            "grouping repair diagnostic."
        )
    )
    ...
    # This file is intentionally absent from the unarmed v0.3 bundle. Validate
    # it before importing the provider SDK or constructing a client.
    if not args.authorization.is_file():
        raise ValueError("v0.3 execution authorization file is required")
        ...
        try:
            ...
            if version != SDK_VERSION:
                ...
                try:
                    ...
                    report = run_instrument_event_repair_diagnostic(
                        contract,
                        parent_failure,
                        authorization,
                        client,
