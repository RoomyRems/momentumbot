...
ALLOWED_IDS = {MICRO_V0_2C_VOLUME_ID, MICRO_V0_2D_CONTEXT_VOLUME_ID}
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Score one volume-factorial cell only after label-blind replay."
    )
    parser.add_argument("--runtime", type=Path, required=True)
    parser.add_argument("--benchmark", type=Path, required=True)
    parser.add_argument("--suite", type=Path, required=True)
    parser.add_argument("--expected-ablation-id", required=True)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()

    if args.expected_ablation_id not in ALLOWED_IDS:
        raise ValueError("unsupported volume-factorial ablation id")
    ...
    if runtime.get("ablation_id") != args.expected_ablation_id:
        raise ValueError("runtime is not the expected factorial cell")
    ...
    if runtime.get("require_lower_pullback_volume") is not False:
        raise ValueError("volume-factorial runtime did not disable the hard volume gate")
    ...
    runtime_first_ordinal = filled_numbers[0] if runtime_available and filled_numbers else None
    ...
    artifact = {
        "artifact_type": "micro_volume_factorial_suite_case_post_replay_score",
        "schema_version": 1,
        ...
        "parent_frozen_policy_fingerprint": runtime.get("parent_frozen_policy_fingerprint"),
        "require_lower_pullback_volume": runtime.get("require_lower_pullback_volume"),
        "volume_rule": runtime.get("volume_rule"),
        "prequalification_context_enabled": runtime.get("prequalification_context_enabled"),
        "structural_context_bars_requested": runtime.get("structural_context_bars_requested"),
        "structural_context_bars_available": runtime.get("structural_context_bars_available"),
        ...
        "price_references_descriptive_only": {
            ...
            "first_runtime_fill_absolute_differences": first_differences,
            "closest_reported_fill_difference_from_first_fill": (
                min(first_differences) if first_differences else None
            ),
