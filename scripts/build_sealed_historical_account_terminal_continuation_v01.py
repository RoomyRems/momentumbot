...
def main():
    ...
    if args.replay:
        ...
        try:
            with output.with_name(output.name + "-progress.jsonl").open("xb") as progress:
                def checkpoint(path_id, pair):
                    row = m.seal({"contract_id": m.CONTRACT_ID,
                        "registration_freeze_content_sha256": args.expected_registration_sha256,
                        "path_id": path_id, "session": pair, "final_panel": False})
                    raw = (json.dumps(row, sort_keys=True, allow_nan=False) + "\n").encode()
                    if progress.write(raw) != len(raw):
                        raise OSError("short progress write")
                    progress.flush()
                    m.os.fsync(progress.fileno())
                with m.binding.OriginalSources(root, paths, expected_registration_sha256=m.PARENT_FREEZE) as sources:
                    result = m.replay_panel(root, sources,
                        expected_registration_sha256=args.expected_registration_sha256, checkpoint=checkpoint)
