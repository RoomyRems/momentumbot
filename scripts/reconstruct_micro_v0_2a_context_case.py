...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Reconstruct one label-blind historical case for the Micro v0.2a "
            "pre-qualification structural-context ablation."
        )
    )
    ...
    ablation = micro_v0_2a_context_ablation()
    ...
    context: dict[str, object] = {
        ...
        "structural_context_bars_requested": ablation.context_bars,
        "structural_context_seconds_requested": ablation.context_seconds,
        "ablation_isolation": (
            "only the structural-history start changes; action remains gated by "
            "actual candidate qualification and frozen v0.1 setup geometry/execution is reused"
        ),
    }
    ...
    context_fetch_start = max(
        session_start,
        qualified_at.floor(f"{ablation.bar_interval_seconds}s")
        - pd.Timedelta(seconds=ablation.context_seconds),
    )

    trades = historical_trades(
        client,
        symbol,
        start=context_fetch_start.to_pydatetime(),
        end=replay_end.to_pydatetime(),
        feed="sip",
        asof=trading_date,
    )
    if trades.empty:
        raise RuntimeError(f"Alpaca returned no SIP trades for {symbol} in ablation window")
    ...
    runtime = prequalification_context_runtime_artifact(result)
    runtime.update(
        {
            "case_id": args.case_id,
            "trading_date": trading_date.isoformat(),
            ...
            "context_fetch_start": context_fetch_start.isoformat(),
            "support_contract": {
