...
def _spec(ablation_id: str):
    if ablation_id == MICRO_V0_2C_VOLUME_ID:
        return micro_v0_2c_volume_ablation()
    if ablation_id == MICRO_V0_2D_CONTEXT_VOLUME_ID:
        return micro_v0_2d_context_volume_ablation()
    raise ValueError(f"unsupported volume factorial cell {ablation_id!r}")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Reconstruct one label-blind Micro volume-factorial runtime cell."
    )
    ...
    runtime_context: dict[str, object] = {
        ...
        "candidate_anchor_scope": (
            ...
            "are deliberately outside this micro setup factorial"
        ),
        "target_qualification": target_row,
        "ablation_id": spec.ablation_id,
        "ablation_fingerprint": spec.fingerprint,
        "ablation_status": spec.status,
        ...
        "volume_rule": spec.volume_rule,
        "prequalification_context_enabled": spec.prequalification_context_enabled,
        "structural_context_bars_requested": spec.context_bars,
        "ablation_isolation": (
            "hard lower-pullback-volume rejection disabled; context is either the "
            "frozen parent boundary or the already-measured v0.2a bound; all other "
            "v0.1 setup and execution rules are unchanged"
        ),
    }
    ...
    if first_acquisition_minute is None or previous_close is None:
        ...
        (output / "runtime-context.json").write_text(
            json.dumps(runtime_context, indent=2, sort_keys=True, default=str) + "\n",
            encoding="utf-8",
        )
        print(json.dumps(runtime, indent=2, sort_keys=True))
        return 0

    acquisition_minute = pd.Timestamp(first_acquisition_minute).floor("min")
    ...
    if spec.prequalification_context_enabled:
        context_seconds = spec.context_bars * spec.context_bar_interval_seconds
        trade_fetch_start = max(
            session_start,
            qualified_at.floor(f"{spec.context_bar_interval_seconds}s")
            - pd.Timedelta(seconds=context_seconds),
        )
    else:
        trade_fetch_start = qualified_at
    ...
    if trades.empty:
        raise RuntimeError(f"Alpaca returned no SIP trades for {symbol} in factorial window")
    ...
    if spec.prequalification_context_enabled:
        result = replay_micro_candidate_with_context_without_hard_volume_gate(
            symbol, bars_10s, trades, **replay_kwargs
        )
    else:
    ...
    runtime = micro_volume_ablation_runtime_artifact(result)
    runtime.update(
        {
            "case_id": args.case_id,
            "trading_date": trading_date.isoformat(),
            "candidate_anchor_source": anchor_source,
            "candidate_anchor_scope": runtime_context["candidate_anchor_scope"],
