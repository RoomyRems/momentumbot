...
def main():
    ...
    if args.check_environment or args.check_execution or args.consume or args.acquire:
        runtime_environment = acq.environment(ROOT)
    ...
    if args.consume:
        ...
        acq.write_json(pre / 'environment.json',runtime_environment)
    ...
    quote.require_exact(marker,expected,'durable v0.2 acquisition consumption')
    quote.require_exact(acq.frozen(pre / 'environment.json'),runtime_environment,'durable pinned environment')
    ...
    for path,name in ((pre / 'consumption.json','consumption.json'),(ROOT / acq.EXECUTION_PATH,'execution.json'),
        ...
        (ROOT / acq.parent.QUOTE_REPORT_PATH,'parent-quote-report.json'),
        (ROOT / acq.ADAPTER_REPORT_PATH,'adapter-verification.json'),(pre / 'environment.json','environment.json')):
    ...
    report = {'contract_id':acq.CONTRACT_ID,'artifact_type':'historical_record_order_input_acquisition_report','provenance':provenance,
        'parent_quote_report_content_sha256':acq.parent.QUOTE_REPORT_CONTENT_SHA,
        'adapter_contract_content_sha256':acq.ADAPTER_CONTRACT_SHA,'requests':[],'requote':None,
        ...
        'acquisition_gate_passed':False,'next_gate':acq.NEXT_GATE}
    ...
    try:
        ...
        if not requote['preflight_passed']:
            ...
            with tempfile.TemporaryDirectory(prefix='historical-execution-input-v02-') as tmp:
    ...
    if report['acquisition_gate_passed']:
        try:
            acq.verify_capture(output,ROOT)
        except Exception:
            report['acquisition_gate_passed'] = False
            report['error'] = 'post_capture_verification_failed'
            report = acq.seal({k:v for k,v in report.items() if k!='content_sha256'})
            acq.write_json(output / 'capture-report.json',report,replace=True)
            files = {p.relative_to(output).as_posix():{'sha256':acq.file_sha(p),'bytes':p.stat().st_size}
                for p in sorted(output.rglob('*')) if p.is_file() and p.name!='capture-inventory.json'}
            acq.write_json(output / 'capture-inventory.json',acq.seal({'contract_id':acq.CONTRACT_ID,
                'provenance':provenance,'files':files,'acquisition_gate_passed':False}),replace=True)
