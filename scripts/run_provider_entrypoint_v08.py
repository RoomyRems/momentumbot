"""Run only the two remaining v0.8 recovery provider entry points.

The audited v0.4 HTTPS/request-budget guard is reused byte-for-byte. The v0.8
...
def resolve_entrypoint(value: str) -> Path:
    ...
    if rendered not in ALLOWED_ENTRYPOINTS:
        raise ValueError("provider entry point is not frozen for v0.8 recovery")
