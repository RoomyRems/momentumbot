...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the separately authorized one-shot Databento INTJ safe "
            "failure classifier."
        )
    )
    ...
    # The child remains unarmed until this exact, separately authorized file
    # exists. Load and validate it before any provider SDK import or client
    # construction.
    if not args.authorization.is_file():
        raise ValueError("v0.2 execution authorization file is required")
        ...
        try:
            ...
            if version != SDK_VERSION:
                ...
                try:
                    ...
                    # The classifier makes exactly one time-series request
                    # after one successful preflight. It never retries.
                    report = run_safe_failure_classifier(
                        contract,
                        parent_failure,
                        authorization,
                        client,
