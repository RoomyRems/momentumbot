"""Run only the three remaining v0.5 recovery provider entry points.

The audited v0.4 HTTPS/request-budget guard is reused byte-for-byte.  The v0.5
allowlist is strictly smaller because Massive membership, identity, and market
discovery are recovered from the exact parent checkpoint and may not be called
again by this child.
"""
...
def resolve_entrypoint(value: str) -> Path:
    ...
    if rendered not in ALLOWED_ENTRYPOINTS:
        raise ValueError("provider entry point is not frozen for v0.5 recovery")
