"""Run only the three remaining v0.6 recovery provider entry points.

The audited v0.4 HTTPS/request-budget guard is reused byte-for-byte. The v0.6
allowlist cannot repeat Massive membership, identity, or market discovery and
uses the identity-compatible float adapter before the unchanged news/scanner
routes.
"""
...
def resolve_entrypoint(value: str) -> Path:
    ...
    if rendered not in ALLOWED_ENTRYPOINTS:
        raise ValueError("provider entry point is not frozen for v0.6 recovery")
