"""Build the provider-free pre-scanner checkpoint for acquisition v0.4."""
...
from momentumbot.research.sealed_historical_source_checkpoint_v01 import (
    ...
    normalize_request_budget_snapshot,
...
def main(argv: list[str] | None = None) -> int:
    ...
    authorization = load_authorization_envelope(args.authorization)
    request_budget = load_json_object(args.request_budget)
    blocked_attempt_ledger = load_json_object(args.blocked_attempt_ledger)
    expected_provenance = {
    ...
    if args.validate_existing:
        ...
        if checkpoint.get("request_budget") != normalize_request_budget_snapshot(
            request_budget,
            authorization,
        ):
            raise ValueError(
                "external request-budget ledger differs from the source checkpoint"
            )
        if checkpoint.get("blocked_attempts") != normalize_blocked_attempt_ledger(
            ...
            raise ValueError(
                "external blocked-attempt ledger differs from the source checkpoint"
            )
        print(
            json.dumps(
                {
    ...
    checkpoint = build_source_checkpoint(
        ...
        blocked_attempt_ledger=blocked_attempt_ledger,
        environment_freeze_path=args.environment_freeze,
        requirements_path=args.requirements,
        checkpoint_output_path=args.output,
        repository=args.repository,
        ...
        json.dumps(
            {
                ...
                "content_sha256": checkpoint["content_sha256"],
                "file_count": checkpoint["inventory"]["file_count"],  # type: ignore[index]
                "provider_calls": 0,
                "stage_count": len(checkpoint["stage_roots"]),  # type: ignore[arg-type]
