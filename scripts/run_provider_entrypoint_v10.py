"""Run only the remaining v0.10 recovery provider entry point.

The audited v0.4 HTTPS/request-budget guard is reused byte-for-byte. The v0.10
allowlist cannot repeat Massive, identity, market, SEC, float, or news
acquisition.  Only the RVOL-aligned scanner-source adapter can run.
"""
...
def resolve_entrypoint(value: str) -> Path:
    ...
    if rendered not in ALLOWED_ENTRYPOINTS:
        raise ValueError("provider entry point is not frozen for v0.10 recovery")
