...
def reacquire_rank_market_inputs(
    ...
    *,
    ...
    minute_adjustment: str = RANK_MINUTE_ADJUSTMENT,
...
def validate_cross_artifact_lineage(
    *,
    ...
    if candidate_payload.get("artifact_id") not in {
        CAUSAL_MARKET_CANDIDATES_ARTIFACT_ID,
        CAUSAL_MARKET_CANDIDATES_V0_3_ARTIFACT_ID,
    }:
    ...
    for date_manifest, expected_id, label in (
        (
            ...
            market_root_manifest.get("artifact_id"),
            "market",
        ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    market_root = args.census_root / args.market_discovery_id
    ...
    scanner_artifact_id = (
        CAUSAL_SCANNER_SNAPSHOT_V0_2_ARTIFACT_ID
        if normalized
        else CAUSAL_SCANNER_SNAPSHOT_ARTIFACT_ID
    )

    output_root = args.output or args.census_root / scanner_artifact_id
    ...
    for value in dates:
        ...
        raw_previous_closes, rank_minutes = reacquire_rank_market_inputs(
            ...
            minute_adjustment=(
                NORMALIZED_RANK_MINUTE_ADJUSTMENT
                if args.gain_basis == SPLIT_CONSISTENT_GAIN_BASIS
                else RANK_MINUTE_ADJUSTMENT
            ),
        )
        rank_minutes = {
        ...
        if args.gain_basis == LEGACY_MIXED_GAIN_BASIS:
        ...
        if source_input_root is not None:
            source_input_manifest = write_scanner_source_input_bundle(
                ...
                scanner_artifact_id=scanner_artifact_id,
                rank_minute_adjustment=(
                    NORMALIZED_RANK_MINUTE_ADJUSTMENT
                    if normalized
                    else RANK_MINUTE_ADJUSTMENT
        ...
        payload, manifest = build_causal_scanner_snapshot_artifacts(
            ...
            scanner_policy=scanner_policy,
            source_market_policy=bundle_manifests["market"]["discovery_policy"],
            artifact_id=scanner_artifact_id,
        )
