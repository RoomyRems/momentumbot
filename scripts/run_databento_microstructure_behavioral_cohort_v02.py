...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the separately authorized v0.2 behavioral cohort diagnostic "
            "under the frozen Databento 0.83 DataFrame repair."
        )
    )
    parser.add_argument("--cohort", type=Path, default=DEFAULT_COHORT)
    parser.add_argument("--protocol", type=Path, default=DEFAULT_PROTOCOL)
    parser.add_argument("--contract", type=Path, default=DEFAULT_CONTRACT)
    parser.add_argument("--parent-failure", type=Path, default=DEFAULT_PARENT_FAILURE)
    parser.add_argument("--authorization", type=Path, default=DEFAULT_AUTHORIZATION)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args(argv)

    if not args.authorization.is_file():
        raise ValueError("behavioral cohort v0.2 authorization file is required")
    cohort = load_cohort(args.cohort)
    protocol = load_protocol(args.protocol)
    contract = load_execution_contract(args.contract)
    parent_failure = load_parent_safe_failure(args.parent_failure)
    ...
    unavailable_args = (cohort, protocol, contract, parent_failure, authorization)
    if run_attempt != "1":
