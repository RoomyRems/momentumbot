"""Verify parents, consume once, then acquire only the registered missing exit sources."""
...
from momentumbot.research import sealed_historical_management_exit_transport_v01 as transport
...
def main():
    ...
    mode.add_argument('--verify-parents', action='store_true')
    mode.add_argument('--verify-capture', action='store_true')
    ...
    if args.check_environment or args.check_execution or args.consume or args.acquire:
        environment = acq.environment(ROOT)
    if args.check_execution or args.consume or args.acquire:
        execution = acq.validate_execution(ROOT, os.environ)
    if args.validate_only:
        print(json.dumps({'valid': True, 'new_requests': 80, 'verified_reuse_requests': 2,
            'unavailable_entries_preserved': 23, 'maximum_http_attempts': 240, 'provider_calls': 0})); return 0
    if args.verify_capture:
        if args.output_root is None: parser.error('capture directory required')
        print(json.dumps(acq.verify_capture(args.output_root, ROOT))); return 0
    ...
    if args.verify_parents:
        print(json.dumps(parents)); return 0
    ...
    for name, value in (('metadata-http-ledger.json', metadata), ('timeseries-http-ledger.json', timeseries),
        ('metadata-ledger.json', acq.seal({'calls': [], 'call_count': 0, 'complete': False})), ('download-ledger.json', acq.download_ledger([]))):
    ...
    try:
        ...
        if not requote['preflight_passed']:
            ...
            raw_temp_removed = False
            with tempfile.TemporaryDirectory(prefix='historical-management-exit-v01-') as temp:
                temporary = Path(temp)
        ...
        report = acq.build_report(rows, metadata, timeseries, requote, provenance, ROOT, error,
            raw_temp_removed=raw_temp_removed)
...
if __name__ == '__main__':
    try: raise SystemExit(main())
        ...
        print('{"status":"failed","sanitized_error":"management exit acquisition validation failed"}', file=sys.stderr)
