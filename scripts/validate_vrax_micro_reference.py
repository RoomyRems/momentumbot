...
def main() -> int:
    ...
    official = client.bars(["VRAX"], timeframe="1Min", start=start, end=end, feed="sip", adjustment="raw", asof=trading_date)["VRAX"]
    _write_frame(trades, args.output / "trades.csv"); _write_frame(bars_10s, args.output / "bars-10s.csv")
    _write_frame(reconstructed_1m, args.output / "reconstructed-1m.csv"); _write_frame(official, args.output / "official-1m.csv")

    levels = {str(level): _first_eligible_at_or_above(trades, level) for level in (5.5, 5.75, 6.0, 6.3, 6.5, 7.0)}
    at_six, at_fill = levels["6.0"], levels["6.3"]
    seconds_6_to_63 = None
    if at_six and at_fill:
        seconds_6_to_63 = (pd.Timestamp(at_fill["timestamp"]) - pd.Timestamp(at_six["timestamp"])).total_seconds()
    ...
    summary = {
        "symbol": "VRAX", "trading_date": trading_date.isoformat(), "candidate_qualified_at": QUALIFIED_AT.isoformat(),
        "window": {"start": start.isoformat(), "end": end.isoformat()}, "feed": "sip", "trade_count": len(trades),
        "ten_second_bar_count": len(bars_10s), "unknown_condition_bars": int((bars_10s["unknown_condition_count"] > 0).sum()),
        "eligible_level_crossings": levels, "seconds_from_6_to_first_eligible_6_30_plus": seconds_6_to_63,
        "observed_pullbacks_after_qualification": pullback_rows,
        "raw_min_price": float(trades["price"].min()), "raw_max_price": float(trades["price"].max()),
        "minute_comparison": _compare_minute_bars(reconstructed_1m, official),
        "provenance": {"micro_bars":"derived_from_historical_sip_trade_prints_using_alpaca_minute_trade_condition_rules","official_bars":"alpaca_historical_sip_1min"},
    }
