"""Provider-free authority, failure, and final gates for recovery v0.8."""
...
from momentumbot.research.sealed_historical_source_acquisition_v08 import (
...
from momentumbot.research.sealed_historical_source_checkpoint_v08 import (
    ...
    build_post_scanner_checkpoint_binding_v08,
...
DEFAULT_AUTHORIZATION = (
    ROOT / "research/strategy/sealed-historical-source-acquisition-v0.8.json"
)
...
def _load_frozen_authority(path: Path) -> tuple[dict[str, object], str, str]:
    ...
    if (
        ...
        raise ValueError("v0.8 authorization constants and payload disagree")
    return authorization, authorization_id, content_sha


def build_consumption_marker_v08(
    *,
    ...
    if authorization_id != expected_id or authorization_content_sha256 != expected_sha:
        raise ValueError("consumption marker authority is not the frozen v0.8 child")
    ...
    if dict(provenance) != canonical:
        raise ValueError("v0.8 consumption provenance is not canonical")
    expected_ref = f"{CONSUMPTION_REF_PREFIX}{authorization_content_sha256}"
    if consumption_ref_name != expected_ref:
        raise ValueError("v0.8 consumption ref is not bound to the authorization")
    if consumption_ref_target_sha != provenance.get("authorization_commit_sha"):
        raise ValueError("v0.8 consumption ref target changed")
    marker: dict[str, object] = {
        "schema_version": 1,
        "artifact_type": "sealed_historical_source_recovery_v0_8_consumption",
        "authorization": {
...
def _safe_blocked_attempts(path: Path | None) -> dict[str, object]:
    ...
    if set(payload) != set(empty):
        raise ValueError("v0.8 blocked-attempt ledger fields changed")
    ...
    if (
        ...
        raise ValueError("v0.8 blocked-attempt ledger is invalid")
    clean_categories: dict[str, int] = {}
    for name, count in categories.items():
        if isinstance(count, bool) or not isinstance(count, int) or count < 0:
            raise ValueError("v0.8 blocked-attempt category count is invalid")
    ...
    for host, count in hosts.items():
        if (
            ...
            raise ValueError("v0.8 blocked-attempt host count is invalid")
        clean_hosts[host] = count
    if sum(clean_categories.values()) != total or sum(clean_hosts.values()) > total:
        raise ValueError("v0.8 blocked-attempt counts are inconsistent")
    return {
        "schema_version": 1,
...
def _completed_dates(source_root: Path, relative: str) -> list[str]:
    ...
    if len(observed) != len(set(observed)) or any(
        ...
        raise ValueError("v0.8 failure stage contains an unexpected date")
    return observed


def build_safe_failure_v08(
    *,
    authorization_id: str,
    authorization_content_sha256: str,
    provenance: Mapping[str, object],
    ...
    _, _, _, expected_id, expected_sha = _authorization_api()
    if authorization_id != expected_id or authorization_content_sha256 != expected_sha:
        raise ValueError("safe failure authority is not the frozen v0.8 child")
    canonical = _strict_provenance(
        repository=provenance.get("repository"),  # type: ignore[arg-type]
        authorization_commit_sha=provenance.get("authorization_commit_sha"),  # type: ignore[arg-type]
        authorization_tree_sha=provenance.get("authorization_tree_sha"),  # type: ignore[arg-type]
    ...
    if dict(provenance) != canonical:
        raise ValueError("v0.8 safe-failure provenance is not canonical")
    ...
    failure: dict[str, object] = {
        "schema_version": 1,
        "artifact_type": "sealed_historical_source_recovery_v0_8_safe_failure",
        "authorization": {
            "authorization_id": authorization_id,
            "authorization_content_sha256": authorization_content_sha256,
        },
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not output_is_outside_source_root(output, source_root):
        raise ValueError("v0.8 report must stay outside the source root")
    checkpoint = load_json_object(checkpoint_path)
    binding = build_post_scanner_checkpoint_binding_v08(
        checkpoint,
    ...
    report = build_acquisition_report_v08(
        authorization_id=authorization_id,
        authorization_content_sha256=content_sha,
