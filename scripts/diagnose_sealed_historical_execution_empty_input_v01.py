"""Consume a separate, bounded diagnostic of the exact missing JVA request."""
...
from momentumbot.research import sealed_historical_execution_empty_diagnostic_v01 as diag
...
def diagnose(transport, output: Path) -> dict:
    ...
    native_projection = output / 'native-projection.jsonl.gz'
    result = {'requote':None,'observation':None,'projection':None,'native_observation':None,
        'native_projection':None,'outcome':None,'error':None,
    ...
    try:
        ...
        native, native_rows = diag.inspect_native_dbn(raw)
        result['native_observation'] = native
        diag.write_json(output/'native-observation.json',native)
        result['native_projection'] = diag.write_projection(native_projection,native_rows)
        ...
        diag.write_json(output/'diagnostic-receipt.json',diag.seal({
            ...
            'native_observation_content_sha256':native['content_sha256'],
            'native_projection':result['native_projection'],
            'projection':result['projection'],'runtime_input_eligible':False}))
        result['outcome'] = diag.observation_outcome(native,observation,native_rows,rows)
        result['diagnostic_evidence_complete'] = result['outcome']['diagnostic_evidence_complete']
        if not result['diagnostic_evidence_complete']:
            result['error'] = result['outcome']['code']
        ...
        if result['native_projection'] is None:
            native_projection.unlink(missing_ok=True)
    finally:
...
def main():
    ...
    if args.check_execution or args.consume or args.diagnose:
        ...
        environment = diag.parent.environment(ROOT)
    ...
    if args.consume:
        ...
        diag.write_json(pre/'environment.json',environment)
    ...
    quote.require_exact(diag.frozen(pre/'environment.json'),environment,'pinned diagnostic environment')
    ...
    for path, name in ((diag.CONTRACT_PATH,'contract.json'),(diag.EXECUTION_PATH,'execution.json'),
        ...
        (diag.parent.parent.QUOTE_REPORT_PATH,'parent-quote-report.json')):
        shutil.copyfile(ROOT/path,output/name)
    shutil.copyfile(pre/'consumption.json',output/'consumption.json')
    shutil.copyfile(pre/'environment.json',output/'environment.json')
    ...
    report = {'contract_id':diag.CONTRACT_ID,'artifact_type':'historical_execution_empty_input_diagnostic',
