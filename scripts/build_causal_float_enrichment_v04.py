...
def _basis_query_window(
    ...
    *,
    ...
    """Cover measure-session fallback only; A(T) comes from market discovery."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.market_discovery_id != CAUSAL_MARKET_DISCOVERY_V0_3_POLICY_ID:
        raise ValueError("causal float v0.2 requires market discovery v0.3")
    ...
    if not all(isinstance(value, str) for value in dates):
        raise ValueError("float-enrichment dates must be strings")
    ...
    date_commitments: list[dict[str, object]] = []
    sec_cache: dict[
        str,
        tuple[
            dict[str, Any] | None,
    ...
    for value in dates:
        ...
        target_relative = discovery_date_manifest.get("files", {}).get(
            "float_target_basis"
        )
        if not isinstance(target_relative, str) or not target_relative:
            raise ValueError(f"{value} market discovery lacks float target basis")
        target_path = Path(target_relative)
        if target_path.is_absolute() or ".." in target_path.parts:
            raise ValueError("float target-basis path escaped market discovery")
        target_pairs, target_basis_payload = load_float_target_basis(
            discovery_root / value / target_path,
            candidate_rows=candidate_rows,
            candidate_payload=candidate_payload,
            expected_trading_date=trading_date,
        )
        target_basis_content_sha256 = str(target_basis_payload["content_sha256"])
        ...
        for candidate in candidate_rows:
            ...
            try:
                ...
                parsed: ParsedCompanyFacts = parse_companyfacts(
                    companyfacts,
                ...
                selected = select_float_evidence(
                    ...
                    first_market_qualified_bar_started_at=qualified_bar_started_at,
                )
            ...
            for tag, requested in tagged_dates:
                observations[f"{tag}:{requested.isoformat()}"] = observe_target_basis(
                    raw,
                    split,
                    requested,
                    target_pair=target_pairs[symbol],
                )
            records.append(
                build_causal_float_record(
                    selected,
        ...
        _write_json(
            records_path,
            _records_payload(trading_date=trading_date, records=records),
        )
        date_manifest = build_causal_float_date_manifest(
            ...
            source_float_target_basis_sha256=target_basis_content_sha256,
            records=records,
        ...
        _write_json(date_manifest_path, date_manifest)
        date_commitments.append(
            {
                "trading_date": value,
                "manifest": f"{value}/manifest.json",
    ...
    root_manifest = build_causal_float_root_manifest(
        ...
        source_market_discovery_bundle_sha256=str(
            discovery_manifest["content_sha256"]
        ),
        date_manifest_commitments=date_commitments,
    ...
    if fatal_provider_errors:
        raise RuntimeError("fatal provider errors prevent a complete float artifact")
    return 0
