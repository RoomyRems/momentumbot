...
from momentumbot.research import sealed_historical_source_checkpoint_v06 as checkpoint
...
from scripts import build_sealed_historical_source_checkpoint_v06 as checkpoint_cli
...
def _budget() -> dict[str, object]:
    return {
        "schema_version": 1,
        "total_attempts": 14_542,
        "by_host": {
            "api.massive.com": 363,
            "data.alpaca.markets": 14_165,
            "data.sec.gov": 14,
        },
    }
...
def _authorization(receipt_hash: str) -> dict[str, object]:
    payload: dict[str, object] = {
        ...
        "request_budget": {
            ...
            "composite_parent_total_attempts": 14_536,
...
class SealedHistoricalSourceCheckpointV06Tests(unittest.TestCase):
    ...
    def _build(self, fixture: dict[str, object]) -> tuple[dict[str, object], dict[str, object]]:
        ...
        with patch.object(
            ...
            result = checkpoint.build_source_checkpoint_v06(
    ...
    def test_round_trip_binds_recovery_diagnostics_and_composite_budget(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(result["request_budget"]["parent_total_attempts"], 14_536)
            ...
            with patch.object(
                ...
                observed = checkpoint.validate_source_checkpoint_v06(
    ...
    def test_composite_budget_cannot_repeat_massive_or_drop_parent_attempts(self) -> None:
        repeated = _budget()
        repeated["by_host"]["api.massive.com"] = 364
        repeated["total_attempts"] = 14_543
        with self.assertRaisesRegex(ValueError, "Massive"):
            checkpoint.normalize_composite_request_budget(repeated)
        below = _budget()
        below["by_host"]["data.sec.gov"] = 11
        below["total_attempts"] = 14_539
    ...
    def test_diagnostic_message_or_file_tamper_fails_rehash(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch.object(
                ...
                checkpoint.validate_source_checkpoint_v06(
                    result,
                    recovery_receipt_path=fixture["receipt_path"],
                    normalization_diagnostic_path=fixture["diagnostic_path"],
                    environment_freeze_path=fixture["freeze"],
    ...
    def test_post_scanner_binding_allows_only_exact_scanner_addition(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch.object(
                ...
                binding = checkpoint.build_post_scanner_checkpoint_binding_v06(
                    result,
