...
WORKFLOW = ROOT / ".github/workflows/sealed-historical-source-acquisition-v06.yml"
...
class SealedHistoricalSourceWorkflowV06Tests(unittest.TestCase):
    @classmethod
    ...
    def test_parent_artifact_is_exact_and_recovery_precedes_provider_access(self) -> None:
        ...
        self.assertEqual(preflight_download["with"]["run-id"], "33516311649")
        verify = self._run(
            "acquire", "Verify exact retained v0.5 failure artifact metadata"
        )
        for value in (
            "9803791643",
            "33516311649",
        ...
        download = self._step(
            "acquire", "Download exact v0.5 failure checkpoint from parent run"
        )["with"]
        self.assertEqual(download["run-id"], "33516311649")
        ...
        first_provider_index = names.index(
            "Preflight all identities and resume target-date float acquisition"
        )
        ...
        self.assertIn("historical_source_recovery_v06", self.text)
    ...
    def test_no_massive_identity_or_market_provider_stage_can_run(self) -> None:
        ...
        self.assertTrue(
            all("scripts/run_provider_entrypoint_v06.py" in run for run in provider_steps)
        )
        self.assertIn("build_causal_float_enrichment_v06.py", provider_steps[0])
    ...
    def test_checkpoint_is_uploaded_before_separate_provider_free_freeze(self) -> None:
        ...
        self.assertIn("run_sealed_historical_source_acquisition_v06.py", freeze_text)
    ...
    def test_action_revisions_and_environment_are_pinned(self) -> None:
        ...
        optimized = self._run(
            "validate", "Repeat v0.6 safety gates under optimized Python"
        )
