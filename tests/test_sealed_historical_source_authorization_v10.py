...
from momentumbot.research import sealed_historical_source_authorization_v10 as auth
from momentumbot.research.sealed_historical_source_checkpoint_v10 import (
    canonical_fingerprint,
)
from scripts.run_sealed_historical_source_acquisition_v10 import (
    _safe_budget,
...
class SealedHistoricalSourceAuthorizationV10Tests(unittest.TestCase):
    def test_authorization_hash_identity_and_recovery_boundary_are_frozen(self) -> None:
        ...
        self.assertEqual(
            ...
            "canonical_split_rank_scanner_source_inputs",
        )
        self.assertTrue(
            payload["recovery_contract"][
                "normalized_parent_source_reused_exactly_except_partial_scanner_tape"
            ]
        )
        self.assertFalse(
            payload["recovery_contract"][
    ...
    def test_composite_budget_and_candidate_repair_do_not_change_strategy(self) -> None:
        ...
        self.assertEqual(budget["composite_parent_total_attempts"], 17_844)
        self.assertEqual(
        ...
        self.assertFalse(repair["candidate_identity_or_float_values_rewritten"])
        self.assertTrue(
        ...
        self.assertTrue(repair["blrx_177_raw_vs_359_dense_timestamp_case_reproduced"])
        self.assertTrue(repair["exact_rvol_projected_to_corresponding_raw_bar_index"])
        self.assertFalse(repair["exact_rvol_fill_or_interpolation_allowed"])
        self.assertFalse(repair["exact_rvol_values_changed"])
        self.assertFalse(repair["partial_parent_scanner_tape_reused"])
        self.assertTrue(
            repair[
        ...
        self.assertEqual(
            payload["provider_entrypoint_contract"]["allowed_entrypoints"],
    ...
    def test_parent_failure_and_registration_bundles_validate_exact_files(self) -> None:
        ...
        self.assertEqual(
            parent["v0_9_failure_audit"]["content_sha256"],
            auth.PARENT_FAILURE_AUDIT_CONTENT_SHA256,
        )
        self.assertTrue(
            parent["v0_9_failure_audit"]["frozen_authority"]
            ["authorization_permanently_consumed"]
        )
        metadata = parent["v0_9_failure_audit"]["preserved_artifacts"][
            "failure_checkpoint"
        ]
        self.assertEqual(
            metadata["zip_sha256"],
            auth.PARENT_FAILURE_CHECKPOINT_ZIP_SHA256,
        )
    ...
    def test_authorization_duplicate_key_and_tamper_fail_closed(self) -> None:
        ...
        changed = copy.deepcopy(payload)
        changed["failed_parent"]["authorization_permanently_consumed"] = False
        changed["content_sha256"] = canonical_fingerprint(
            {key: value for key, value in changed.items() if key != "content_sha256"}
        )
        with self.assertRaisesRegex(ValueError, "frozen hash|failed-parent"):
            auth.validate_authorization(changed)
    ...
    def test_registration_detects_one_bound_file_change(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            target = root / auth.REGISTRATION_ARTIFACT_PATHS["rvol_alignment"]
            target.write_bytes(target.read_bytes() + b"\n")
            with patch.object(auth, "ROOT", root), self.assertRaisesRegex(
                ValueError, "rvol_alignment hash changed"
            ):
    ...
    def test_consumption_marker_binds_exact_tag_target_and_attempt(self) -> None:
        ...
        marker = build_consumption_marker_v10(
