...
from momentumbot.research import sealed_historical_source_recovery_v10 as recovery


class SealedHistoricalSourceRecoveryV10Tests(unittest.TestCase):
    ...
    def test_materialization_copies_only_validated_source_and_seeds_budget(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            partial = parent / "source/causal-scanner-source-inputs-v0.2"
            partial.mkdir()
            (partial / "partial").write_text("ignored", encoding="utf-8")
            output = root / "output"
            ...
            with patch.object(
                ...
                side_effect=[parent_commitment, recovered_commitment],
            ):
            ...
            self.assertFalse(
                (output / "causal-scanner-source-inputs-v0.2").exists()
            )
            self.assertEqual(
