...
from scripts import run_provider_entrypoint_v10 as wrapper


class ProviderEntrypointV10Tests(unittest.TestCase):
    def test_allowlist_contains_only_remaining_scanner_route(self) -> None:
        self.assertEqual(
    ...
    def test_main_installs_guard_before_running_exact_file(self) -> None:
        target = wrapper.ROOT / "scripts/build_causal_scanner_snapshot_v10.py"
        ...
        with patch.object(
            ...
            wrapper.main(["scripts/build_causal_scanner_snapshot_v10.py", "--help"])
    ...
    def test_symlink_entrypoint_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            scripts.mkdir()
            target = scripts / "build_causal_scanner_snapshot_v10.py"
            target.symlink_to(wrapper.ROOT / "scripts/build_causal_scanner_snapshot_v10.py")
    ...
    def test_exact_workflow_script_invocations_resolve_peer_modules(self) -> None:
        provider = subprocess.run(
            [sys.executable, "scripts/run_provider_entrypoint_v10.py", "--help"],
            cwd=wrapper.ROOT,
            text=True,
            capture_output=True,
            check=False,
        ...
        self.assertEqual(scanner_help.returncode, 0, scanner_help.stderr)
        self.assertNotIn("ModuleNotFoundError", scanner_help.stderr)
