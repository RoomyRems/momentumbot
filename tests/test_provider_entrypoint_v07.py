...
from scripts import run_provider_entrypoint_v07 as wrapper


class ProviderEntrypointV07Tests(unittest.TestCase):
    def test_allowlist_contains_only_remaining_recovery_routes(self) -> None:
        self.assertEqual(
            wrapper.ALLOWED_ENTRYPOINTS,
            {
                "scripts/build_causal_news_enrichment_v07.py",
                "scripts/build_causal_scanner_snapshot_v07.py",
            },
        )
        ...
        self.assertNotIn("data.sec.gov", wrapper.CHILD_REQUEST_HOSTS)
    ...
    def test_main_installs_guard_before_running_exact_file(self) -> None:
        target = wrapper.ROOT / "scripts/build_causal_news_enrichment_v07.py"
        ...
        with patch.object(
            ...
            wrapper.main(["scripts/build_causal_news_enrichment_v07.py", "--help"])
    ...
    def test_symlink_entrypoint_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            scripts.mkdir()
            target = scripts / "build_causal_news_enrichment_v07.py"
            target.symlink_to(wrapper.ROOT / "scripts/build_causal_news_enrichment_v07.py")
    ...
    def test_exact_workflow_script_invocations_resolve_peer_modules(self) -> None:
        provider = subprocess.run(
            [sys.executable, "scripts/run_provider_entrypoint_v07.py", "--help"],
            cwd=wrapper.ROOT,
            text=True,
            capture_output=True,
            check=False,
        ...
        self.assertEqual(news_help.returncode, 0, news_help.stderr)
        self.assertNotIn("ModuleNotFoundError", news_help.stderr)
