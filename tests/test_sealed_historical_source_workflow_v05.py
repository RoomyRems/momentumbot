...
WORKFLOW = ROOT / ".github/workflows/sealed-historical-source-acquisition-v05.yml"
...
class SealedHistoricalSourceWorkflowV05Tests(unittest.TestCase):
    @classmethod
    ...
    def test_parent_artifact_is_exact_and_recovery_precedes_provider_access(self) -> None:
        ...
        self.assertEqual(preflight_download["with"]["run-id"], "33468687163")
        verify = self._run(
            "acquire", "Verify exact retained v0.4 failure artifact metadata"
        )
        for value in (
            "9790775145",
            "33468687163",
        ...
        download = self._step(
            "acquire", "Download exact v0.4 failure checkpoint from parent run"
        )["with"]
        self.assertEqual(download["run-id"], "33468687163")
        ...
        first_provider_index = names.index(
            "Resume candidate-contained target-date float acquisition"
        )
    ...
    def test_no_massive_identity_or_market_provider_stage_can_run(self) -> None:
        forbidden = (
        ...
        provider_steps = [
            self._run("acquire", "Resume candidate-contained target-date float acquisition"),
        ...
        self.assertTrue(
            all("scripts/run_provider_entrypoint_v05.py" in run for run in provider_steps)
        )
        self.assertIn("build_causal_float_enrichment_v05.py", provider_steps[0])
        self.assertIn("--sanitized-normalization-diagnostics", provider_steps[0])
    ...
    def test_checkpoint_is_uploaded_before_separate_provider_free_freeze(self) -> None:
        ...
        self.assertIn("run_sealed_historical_source_acquisition_v05.py", freeze_text)
    ...
    def test_action_revisions_and_environment_are_pinned(self) -> None:
        ...
        optimized = self._run(
            "validate", "Repeat v0.5 safety gates under optimized Python"
        )
