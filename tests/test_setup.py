...
class SetupTests(unittest.TestCase):
    ...
    def test_deep_retrace_rejected(self):
        bars = self.bars.copy(); bars.iloc[-1, bars.columns.get_loc("low")] = 5.3; bars.iloc[-1, bars.columns.get_loc("close")] = 5.4
        self.assertIsNone(build_first_pullback_plan("TEST", bars, self.profile))

    def test_heavy_pullback_volume_rejected(self):
        bars = self.bars.copy(); bars.iloc[-2:, bars.columns.get_loc("volume")] = 2_000_000
        self.assertIsNone(build_first_pullback_plan("TEST", bars, self.profile))

    def test_break_below_vwap_or_ema_rejected(self):
        bars = self.bars.copy(); bars.iloc[-1, bars.columns.get_loc("low")] = 4.2; bars.iloc[-1, bars.columns.get_loc("close")] = 6.08
        self.assertIsNone(build_first_pullback_plan("TEST", bars, self.profile))

    def test_future_bar_does_not_change_existing_plan(self):
        first = build_first_pullback_plan("TEST", self.bars, self.profile)
        future = frame([(6.08, 20.0, 1.0, 10.0, 9_000_000)], start=str(self.bars.index[-1] + pd.Timedelta(minutes=1)))
        combined = pd.concat([self.bars, future])
        second = build_first_pullback_plan("TEST", combined.loc[: self.bars.index[-1]], self.profile)
        self.assertEqual(first, second)

    def test_prior_bars_warm_indicators_without_entering_session_vwap_geometry(self):
        warmup = self.bars.iloc[:-7]
        session = self.bars.iloc[-7:]
        cold = evaluate_first_pullback_plan("TEST", session, self.profile)
        warmed = evaluate_first_pullback_plan("TEST", session, self.profile, indicator_warmup=warmup)
        self.assertEqual(cold.reason, "insufficient_history")
        self.assertNotEqual(warmed.reason, "insufficient_history")
