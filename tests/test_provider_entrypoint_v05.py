...
from scripts import run_provider_entrypoint_v05 as wrapper


class ProviderEntrypointV05Tests(unittest.TestCase):
    ...
    def test_parent_market_and_massive_entrypoints_are_prohibited(self) -> None:
        for value in (
    ...
    def test_symlink_entrypoint_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            scripts.mkdir()
            target = scripts / "build_causal_float_enrichment_v05.py"
            target.symlink_to(wrapper.ROOT / "scripts/build_causal_float_enrichment_v05.py")
    ...
    def test_exact_workflow_script_invocations_resolve_peer_modules(self) -> None:
        provider = subprocess.run(
            [sys.executable, "scripts/run_provider_entrypoint_v05.py", "--help"],
            cwd=wrapper.ROOT,
            text=True,
            capture_output=True,
            check=False,
