...
from momentumbot.research import sealed_historical_source_authorization_v09 as auth
from momentumbot.research.sealed_historical_source_checkpoint_v09 import (
    canonical_fingerprint,
)
from scripts.run_sealed_historical_source_acquisition_v09 import (
    _safe_budget,
...
class SealedHistoricalSourceAuthorizationV09Tests(unittest.TestCase):
    ...
    def test_parent_failure_and_registration_bundles_validate_exact_files(self) -> None:
        ...
        self.assertEqual(
            parent["v0_8_failure_audit"]["content_sha256"],
            auth.FAILED_CHILD_FAILURE_AUDIT_CONTENT_SHA256,
        )
        self.assertFalse(
            parent["v0_8_failure_audit"]["frozen_authority"]
            ["authorization_permanently_consumed"]
        )
        metadata = parent["v0_8_failure_audit"][
            "independently_verified_parent_artifact"
        ]
        self.assertEqual(
            metadata["digest"],
        ...
        self.assertTrue(metadata["digest_matches_downloaded_zip_sha256"])
        self.assertEqual(metadata["archive_integrity_check"], "passed")
    ...
    def test_consumption_marker_binds_exact_tag_target_and_attempt(self) -> None:
        ...
        marker = build_consumption_marker_v09(
