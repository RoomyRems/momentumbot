...
from scripts import build_sealed_historical_source_checkpoint_v10 as checkpoint_cli
...
def _budget() -> dict[str, object]:
    return {
        "schema_version": 1,
        "total_attempts": 17_850,
        "by_host": {
            "api.massive.com": 363,
            "data.alpaca.markets": 16_159,
            "data.sec.gov": 1_328,
        },
    }
...
class SealedHistoricalSourceCheckpointV10Tests(unittest.TestCase):
    ...
    def _build(self, fixture: dict[str, object]) -> tuple[dict[str, object], dict[str, object]]:
        ...
        with patch.object(
            ...
            result = checkpoint.build_source_checkpoint_v10(
    ...
    def test_round_trip_binds_recovery_diagnostics_and_composite_budget(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(result["request_budget"]["parent_total_attempts"], 17_844)
            ...
            with patch.object(
                ...
                observed = checkpoint.validate_source_checkpoint_v10(
    ...
    def test_composite_budget_cannot_repeat_massive_sec_or_drop_parent_attempts(self) -> None:
        ...
        repeated["total_attempts"] = 17_851
        ...
        repeated_sec["total_attempts"] = 17_851
    ...
    def test_diagnostic_message_or_file_tamper_fails_rehash(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch.object(
                ...
                checkpoint.validate_source_checkpoint_v10(
                    result,
                    recovery_receipt_path=fixture["receipt_path"],
                    normalization_diagnostic_path=fixture["diagnostic_path"],
                    environment_freeze_path=fixture["freeze"],
    ...
    def test_post_scanner_binding_allows_only_exact_scanner_addition(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch.object(
                ...
                binding = checkpoint.build_post_scanner_checkpoint_binding_v10(
                    result,
