...
from momentumbot.research import sealed_historical_source_artifact_metadata_v10 as metadata
...
LIVE_GITHUB_ARTIFACT_METADATA = {
    ...
    "digest": (
        "sha256:0db44af6ffe695642444e384378faf3d"
        "fb3b6be8e059c0dca7bc0ee77d589244"
    ),
    "size_in_bytes": 44_337_250,
    "expired": False,
    "workflow_run": {"id": 33_577_895_166},
    "created_at": "2026-09-02T01:10:27Z",
    "expires_at": "2026-12-01T01:04:13Z",
}
DOWNLOADED_ZIP_SHA256 = (
    "0db44af6ffe695642444e384378faf3d"
    "fb3b6be8e059c0dca7bc0ee77d589244"
)
...
class SealedHistoricalSourceArtifactMetadataV10Tests(unittest.TestCase):
    ...
    def test_exact_metadata_builds_hash_bound_provider_free_receipt(self) -> None:
        receipt = metadata.validate_parent_artifact_metadata_v10(_payload())
        body = dict(receipt)
        ...
        for field, value in cases.items():
            with self.subTest(field=field):
                ...
                with self.assertRaisesRegex(ValueError, rf"field {field} changed") as ctx:
                    metadata.validate_parent_artifact_metadata_v10(changed)
    ...
    def test_type_confusion_duplicate_keys_nonfinite_and_symlink_fail_closed(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ValueError, "duplicate JSON key"):
                metadata.load_and_validate_parent_artifact_metadata_v10(duplicate)
            nonfinite = root / "nonfinite.json"
            nonfinite.write_text('{"id":NaN}', encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "non-finite JSON"):
                metadata.load_and_validate_parent_artifact_metadata_v10(nonfinite)
            valid = root / "valid.json"
            valid.write_text(json.dumps(_payload()), encoding="utf-8")
            link = root / "link.json"
            link.symlink_to(valid)
            with self.assertRaisesRegex(ValueError, "regular file"):
                metadata.load_and_validate_parent_artifact_metadata_v10(link)
