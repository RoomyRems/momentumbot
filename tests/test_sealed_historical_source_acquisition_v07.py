...
from momentumbot.research import sealed_historical_source_acquisition_v07 as acquisition
from momentumbot.research import sealed_historical_source_checkpoint_v07 as checkpoint
...
def _build() -> dict[str, object]:
    with patch.object(
        ...
        return acquisition.build_acquisition_report_v07(
            authorization_id=checkpoint.AUTHORIZATION_ID,
            authorization_content_sha256=AUTHORIZATION_SHA,
...
class SealedHistoricalSourceAcquisitionV07Tests(unittest.TestCase):
    def test_report_cross_binds_parent_diagnostic_budget_and_final_tree(self) -> None:
        ...
        self.assertEqual(report["request_budget"]["parent_total_attempts"], 17_540)
    ...
    def test_report_rejects_checkpoint_tree_budget_and_recovery_mismatch(self) -> None:
        for field, value, pattern in (
            ...
            ("request_budget", {**_budget(), "total_attempts": 17_547}, "inconsistent|differs"),
        ):
            kwargs = {
                "authorization_id": checkpoint.AUTHORIZATION_ID,
                "authorization_content_sha256": AUTHORIZATION_SHA,
            ...
            with patch.object(
                ...
                acquisition.build_acquisition_report_v07(**kwargs)
    ...
    def test_self_hash_and_causal_attestation_are_fail_closed(self) -> None:
        ...
        with patch.object(
            ...
            acquisition.validate_acquisition_report_v07(changed)
