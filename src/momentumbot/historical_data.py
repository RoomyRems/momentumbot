    ...
    *,
    ...
    for symbol in superset:
        ...
        average = _average_volume_before(
            daily, trading_date, sessions=profile.rvol_lookback_sessions
        )
...
def write_discovery(result: DiscoveryResult, root: Path, *, trading_date: date) -> None:
    ...
    pd.DataFrame([asdict(row) for row in result.rows]).to_csv(
        root / "discovery.csv", index=False
    )
    manifest = {
        "schema_version": 2,
        ...
        "notes": [
            ...
            "Full-day high is used only to decide what data to acquire and is never a strategy feature.",
        ],
    }
...
def write_reference_case(
    *,
    ...
    if rvol_curve is not None:
        rvol_curve.rename("relative_volume").to_csv(root / "rvol.csv", index_label="timestamp")
    context_row = {
        "symbol": symbol,
