"""Versioned checkpoint adapter for sealed historical source recovery v0.6.

The source layout and scanner-freeze contract are unchanged from v0.5.  This
adapter reuses that audited implementation while rebinding its immutable
authority, parent-recovery receipt, workflow, and cumulative request seed.
"""
...
ARTIFACT_TYPE = "sealed_historical_label_blind_source_recovery_checkpoint_v0_6"
...
@contextmanager
def _v06_contract() -> Iterator[None]:
...
def validate_authorization_envelope_v06(
...
def load_authorization_envelope_v06(path: str | Path) -> dict[str, object]:
    with _v06_contract():
...
def normalize_composite_request_budget(
    ...
    with _v06_contract():
        return parent.normalize_composite_request_budget(payload)


def build_source_checkpoint_v06(**kwargs: object) -> dict[str, object]:
    with _v06_contract():
        return parent.build_source_checkpoint_v05(**kwargs)  # type: ignore[arg-type]


def validate_source_checkpoint_v06(
...
def build_post_scanner_checkpoint_binding_v06(
    checkpoint: Mapping[str, object],
    **kwargs: object,
) -> dict[str, object]:
    with _v06_contract():
...
def validate_post_scanner_checkpoint_binding_v06(
