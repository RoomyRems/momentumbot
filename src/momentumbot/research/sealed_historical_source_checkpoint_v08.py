"""Versioned checkpoint adapter for sealed historical source recovery v0.8.

The final pre/post-scanner layout remains 706/767 files because v0.8 recovers
...
ARTIFACT_TYPE = "sealed_historical_label_blind_source_recovery_checkpoint_v0_8"
...
RECOVERY_RECEIPT_CONTENT_SHA256 = (
    "e25d892acb469309674384e3731291e2c7a44a6ddbbee13bf4fb5cbe285ce4c1"
)
...
@contextmanager
def _v08_contract() -> Iterator[None]:
...
def validate_authorization_envelope_v08(
...
def load_authorization_envelope_v08(path: str | Path) -> dict[str, object]:
    with _v08_contract():
...
def normalize_composite_request_budget(
    ...
    for host in ("api.massive.com", "data.sec.gov"):
        if normalized["by_host"].get(host) != PARENT_REQUEST_BUDGET["by_host"][host]:
            raise ValueError(f"v0.8 composite request budget repeated prohibited {host}")
    return normalized


def build_source_checkpoint_v08(**kwargs: object) -> dict[str, object]:
    with _v08_contract():
        return parent.build_source_checkpoint_v05(**kwargs)  # type: ignore[arg-type]


def validate_source_checkpoint_v08(
...
def build_post_scanner_checkpoint_binding_v08(
    checkpoint: Mapping[str, object],
    **kwargs: object,
) -> dict[str, object]:
    with _v08_contract():
...
def validate_post_scanner_checkpoint_binding_v08(
...
__all__ = [
    ...
    "build_source_checkpoint_v08",
    "canonical_fingerprint",
    "load_authorization_envelope_v08",
    ...
    "validate_source_checkpoint_v08",
