"""Authorization-only contract for sealed source recovery v0.8."""
...
from momentumbot.research.sealed_historical_source_checkpoint_v08 import (
    ...
    validate_authorization_envelope_v08,
)
...
FAILED_CHILD_AUTHORIZATION_CONTENT_SHA256 = (
    "c45dfa3276b15465db3afca3f3370caea19fe55337f92981fa421a7a734c0221"
)
FAILED_CHILD_FAILURE_AUDIT_CONTENT_SHA256 = (
    "9422a1e2ef3cfef5c90737812b5e876188e8630b4a21b88058538c16efd23f3b"
)
FAILED_CHILD_RUN_ID = 33530672018
...
FAILED_CHILD_FAILURE_AUDIT_PATH = Path(
    "research/data-audits/"
    "sealed-historical-source-acquisition-v0.7-run-33530672018-failure-2026-09-01.json"
)
...
EXPECTED_DISPATCHER_WORKFLOW_REF = (
    ...
    "sealed-historical-source-acquisition-v08.yml@refs/heads/main"
)
CONSUMPTION_TAG_PREFIX = (
    "refs/tags/sealed-historical-source-acquisition-v08-consumed-"
)

REGISTRATION_ARTIFACT_PATHS = {
    "acquisition_validator": Path(
        "src/momentumbot/research/sealed_historical_source_acquisition_v08.py"
    ),
    "alpaca_provider": Path("src/momentumbot/providers/alpaca.py"),
    "authorization": AUTHORIZATION_PATH,
    "authorization_validator": Path(
        "src/momentumbot/research/sealed_historical_source_authorization_v08.py"
    ),
    "artifact_metadata_validator": Path(
        "src/momentumbot/research/sealed_historical_source_artifact_metadata_v08.py"
    ),
    "artifact_metadata_runner": Path(
        "scripts/validate_parent_artifact_metadata_v08.py"
    ),
    "checkpoint_builder": Path(
        "scripts/build_sealed_historical_source_checkpoint_v08.py"
    ),
    "checkpoint_validator": Path(
        "src/momentumbot/research/sealed_historical_source_checkpoint_v08.py"
    ),
    "documentation": Path(
        "docs/research/sealed_historical_source_acquisition_v08.md"
    ),
    ...
    "failed_child_registration": Path(
        "research/data-audits/"
        "sealed-historical-source-acquisition-v0.7-registration-2026-09-01.json"
    ),
    ...
    "downstream_identity_compatibility": Path(
        "src/momentumbot/historical_float_identity_v08.py"
    ),
    ...
    "news_adapter": Path("scripts/build_causal_news_enrichment_v08.py"),
    ...
    "provider_wrapper": Path("scripts/run_provider_entrypoint_v08.py"),
    "provider_transport_guard": Path("scripts/run_provider_entrypoint_v04.py"),
    "recovery_runner": Path("scripts/validate_sealed_historical_source_recovery_v08.py"),
    "recovery_validator": Path(
        "src/momentumbot/research/sealed_historical_source_recovery_v08.py"
    ),
    "request_budget": Path("src/momentumbot/providers/request_budget.py"),
    "requirements": Path("requirements-sealed-source-v04.txt"),
    "runner": Path("scripts/run_sealed_historical_source_acquisition_v08.py"),
    "scanner_adapter": Path("scripts/build_causal_scanner_snapshot_v08.py"),
...
def validate_authorization(payload: Mapping[str, object]) -> None:
    validated = validate_authorization_envelope_v08(payload)
    if validated.get("content_sha256") != AUTHORIZATION_CONTENT_SHA256:
        raise ValueError("historical source v0.8 differs from frozen hash")
    if validated.get("dispatcher_contract") != {
        ...
        raise ValueError("v0.8 dispatcher contract changed")
    if validated.get("failed_child") != {
        "authorization_commit_sha": "dfe63ebf716f288455ab9a2e6eed1a3695c84439",
        "authorization_content_sha256": FAILED_CHILD_AUTHORIZATION_CONTENT_SHA256,
        "authorization_id": "sealed-historical-source-acquisition-v0.7",
        "authorization_permanently_consumed": False,
        "authorization_tree_sha": "b2d67e787ad12e93ec81e2bf2c72a55110f14bbe",
        "consumption_tag_created": False,
        "dispatcher_workflow_sha": "d872d56c5840d8542c281b105ec0d32ff8baacae",
        "failure_audit_content_sha256": FAILED_CHILD_FAILURE_AUDIT_CONTENT_SHA256,
        "provider_calls": 0,
        "workflow_run_attempt": 1,
        "workflow_run_id": FAILED_CHILD_RUN_ID,
    }:
        raise ValueError("v0.8 failed-child provenance changed")
    if validated.get("reproducibility_environment_contract") != {
        ...
        raise ValueError("v0.8 reproducibility environment contract changed")
    ...
    if not isinstance(provider, Mapping) or provider.get("child_network_hosts") != [
        ...
        raise ValueError("v0.8 child provider network boundary changed")
    if validated.get("repair_boundary") != {
        ...
        raise ValueError("v0.8 downstream identity repair boundary changed")
...
def validate_parent_bundle() -> dict[str, object]:
    ...
    if (
        ...
        raise ValueError("v0.7 authorization parent changed")
    ...
    if (
        ...
        != "dfe63ebf716f288455ab9a2e6eed1a3695c84439"
        or failed_authority.get("authorization_tree_sha")
        != "b2d67e787ad12e93ec81e2bf2c72a55110f14bbe"
        or failed_child.get("conclusion")
        != "fail_closed_parent_artifact_metadata_preflight_unobservable_comparison"
    ):
        raise ValueError("v0.7 failure parent provenance changed")
    if (
        ...
        or failed_repair.get("v0_7_may_be_rerun") is not False
        ...
        raise ValueError("v0.7 safe pre-consumption failure boundary changed")
    return {
        "v0_6_authorization": parent_authorization,
        "v0_6_failure_audit": failure,
        "v0_7_authorization": failed_child_authorization,
        "v0_7_failure_audit": failed_child,
    }
...
def validate_registration_bundle() -> dict[str, object]:
    ...
    if path.is_symlink() or not path.is_file():
        raise ValueError("v0.8 registration audit must be a regular file")
    ...
    if set(audit) != expected_keys:
        raise ValueError("v0.8 registration audit fields changed")
    _validate_self_hash(
        audit,
        expected=str(audit.get("content_sha256")),
        label="v0.8 registration audit",
    )
    if (
        audit.get("schema_version") != 1
        or audit.get("artifact_type")
        != "provider_free_sealed_historical_source_recovery_v0_8_registration"
        ...
        raise ValueError("v0.8 registration identity changed")
    if audit.get("frozen_parent") != {
        ...
        raise ValueError("v0.8 registration parent changed")
    if audit.get("repair") != {
        ...
        raise ValueError("v0.8 registration repair boundary changed")
    if audit.get("authority_boundary") != {
        ...
        raise ValueError("v0.8 registration authority boundary changed")
    if audit.get("causal_attestation") != {
        ...
        raise ValueError("v0.8 registration causal attestation changed")
    artifacts = audit.get("artifacts")
    if not isinstance(artifacts, Mapping) or set(artifacts) != set(
        REGISTRATION_ARTIFACT_PATHS
    ):
        raise ValueError("v0.8 registration artifact census changed")
    for label, relative in REGISTRATION_ARTIFACT_PATHS.items():
        ...
        if (
            ...
            raise ValueError(f"v0.8 registration artifact {label} changed")
        absolute = ROOT / relative
        if absolute.is_symlink() or not absolute.is_file():
            raise ValueError(f"v0.8 registration artifact {label} is not regular")
        if entry.get("file_sha256") != _file_sha256(absolute):
            raise ValueError(f"v0.8 registration artifact {label} hash changed")
    return audit
