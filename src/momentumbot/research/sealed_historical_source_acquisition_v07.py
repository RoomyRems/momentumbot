"""Strict provider-free final report for sealed source recovery v0.7."""
...
from momentumbot.research.sealed_historical_source_checkpoint_v07 import (
    ...
    validate_post_scanner_checkpoint_binding_v07,
)


SCHEMA_VERSION = 1
ARTIFACT_TYPE = "sealed_historical_source_recovery_v0_7_result"
...
def _frozen_authorization_content_sha256() -> str:
    ...
    if _SHA256.fullmatch(AUTHORIZATION_CONTENT_SHA256) is None:
        raise ValueError("frozen v0.7 authorization hash is invalid")
...
def build_acquisition_report_v07(
    *,
    ...
    if (
        ...
        raise ValueError("report authority is not the frozen v0.7 child")
    checkpoint = validate_post_scanner_checkpoint_binding_v07(
    ...
    if retained > MAX_RETAINED_BYTES:
        raise ValueError("v0.7 retained-byte ceiling exceeded")
    provenance = _workflow_provenance_v07(
        repository=repository,
        authorization_commit_sha=authorization_commit_sha,
        authorization_tree_sha=authorization_tree_sha,
        dispatcher_workflow_sha=dispatcher_workflow_sha,
    ...
    if checkpoint.get("authorization") != {
        ...
        raise ValueError("v0.7 checkpoint is bound to another authorization")
    if checkpoint.get("request_budget") != budget:
        raise ValueError("v0.7 checkpoint request accounting differs from report")
    if checkpoint.get("provenance") != provenance:
        raise ValueError("v0.7 checkpoint provenance differs from report")
    if (
        ...
        raise ValueError("v0.7 checkpoint final tree differs from source summary")
    ...
    validate_acquisition_report_v07(report)
    return report


def validate_acquisition_report_v07(report: Mapping[str, object]) -> None:
    expected_keys = {
        "schema_version",
        "artifact_type",
        "authorization_id",
    ...
    if set(report) != expected_keys:
        raise ValueError("v0.7 acquisition report fields changed")
    ...
    if not isinstance(claimed, str) or _SHA256.fullmatch(claimed) is None or claimed != canonical_fingerprint(unsigned):
        raise ValueError("v0.7 acquisition report hash mismatch")
    if (
        ...
        raise ValueError("unsupported v0.7 acquisition report")
    checkpoint = report.get("source_checkpoint")
    if not isinstance(checkpoint, Mapping):
        raise ValueError("v0.7 acquisition checkpoint binding is missing")
    checkpoint = validate_post_scanner_checkpoint_binding_v07(checkpoint)
    if report.get("parent_recovery") != checkpoint.get("recovery"):
        raise ValueError("v0.7 parent recovery binding differs from checkpoint")
    if report.get("normalization_diagnostics") != checkpoint.get(
        "normalization_diagnostics"
    ):
        raise ValueError("v0.7 normalization diagnostics differ from checkpoint")
    summary = report.get("source_summary")
    if not isinstance(summary, Mapping):
        raise ValueError("v0.7 source summary is missing")
    validate_source_summary_v04(
        summary,
        expected_manifest_paths=expected_manifest_paths_v04(),
        expected_source_file_count=EXPECTED_POST_SCANNER_FILE_COUNT,
    ...
    if not isinstance(budget, Mapping):
        raise ValueError("v0.7 request budget is missing")
    ...
    if dict(budget) != normalized_budget or checkpoint.get("request_budget") != budget:
        raise ValueError("v0.7 composite request accounting differs")
    ...
    if not isinstance(retention, Mapping) or set(retention) != {
        ...
        raise ValueError("v0.7 retention boundary changed")
    ...
    if not isinstance(provenance, Mapping):
        raise ValueError("v0.7 workflow provenance is missing")
    normalized_provenance = _workflow_provenance_v07(**dict(provenance))  # type: ignore[arg-type]
    ...
    if (
        ...
        raise ValueError("v0.7 report final source commitment differs")
    if report.get("cost") != {
        "incremental_provider_cost_usd": "0",
        "databento_called": False,
    }:
        raise ValueError("v0.7 cost boundary changed")
    if report.get("causal_attestation") != {
        ...
        raise ValueError("v0.7 causal attestation changed")
    if (
        ...
        raise ValueError("v0.7 final gate changed")


def load_acquisition_report_v07(path: str | Path) -> dict[str, object]:
    from momentumbot.research.sealed_historical_source_checkpoint_v01 import (
        load_json_object,
    )

    payload = load_json_object(Path(path))
    validate_acquisition_report_v07(payload)
    return payload
...
__all__ = [
    ...
    "load_acquisition_report_v07",
    "validate_acquisition_report_v07",
]
