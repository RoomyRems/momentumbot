"""Strict provider-free final report for sealed source recovery v0.12."""
...
SCHEMA_VERSION = 1
ARTIFACT_TYPE = "sealed_historical_source_freeze_recovery_v0_12_result"
...
def validate_recovery_environment_pair_v12(
    *,
    ...
    if child != parent:
        raise ValueError("v0.12 third-party environment differs from v0.10")
    return {
        "schema_version": 1,
...
def _validate_environment_comparison(
    ...
    if set(value) != expected_keys:
        raise ValueError("v0.12 environment comparison fields changed")
    if (
        ...
        raise ValueError("v0.12 parent environment comparison changed")
    for field in (
        "child_environment_freeze_sha256",
        "third_party_environment_sha256",
    ):
        ...
        if not isinstance(observed, str) or re.fullmatch(r"[0-9a-f]{64}", observed) is None:
            raise ValueError(f"v0.12 environment {field} is invalid")
    ...
    if not isinstance(child_commit, str) or _GIT_SHA.fullmatch(child_commit) is None:
        raise ValueError("v0.12 child environment commit is invalid")
    return dict(value)


def build_recovery_report_v12(
    *,
    ...
    if (
        ...
        raise ValueError("report authority is not the frozen v0.12 child")
    ...
    if (
        ...
        raise ValueError("v0.12 parent checkpoint binding changed")
    validate_source_summary_v04(
        source_summary,
    ...
    if (
        ...
        raise ValueError("v0.12 retained bytes are invalid")
    if (
        ...
        raise ValueError("v0.12 final source differs from parent binding")
    provenance = _workflow_provenance_v12(
        repository=repository,
        authorization_commit_sha=authorization_commit_sha,
        authorization_tree_sha=authorization_tree_sha,
        dispatcher_workflow_sha=dispatcher_workflow_sha,
    ...
    if environment.get("child_project_commit_sha") != provenance.get(
        ...
        raise ValueError("v0.12 environment commit differs from authorization")
    ...
    validate_recovery_report_v12(report)
    return report


def validate_recovery_report_v12(report: Mapping[str, object]) -> None:
    expected_keys = {
        "schema_version",
        "artifact_type",
        "authorization_id",
    ...
    if set(report) != expected_keys:
        raise ValueError("v0.12 recovery report fields changed")
    body = dict(report)
    claimed = body.pop("content_sha256", None)
    if claimed != canonical_fingerprint(body):
        raise ValueError("v0.12 recovery report hash mismatch")
    if (
        ...
        raise ValueError("unsupported v0.12 recovery report")
    ...
    if (
        ...
        raise ValueError("v0.12 parent checkpoint is missing")
    binding = parent.get("binding")
    if not isinstance(binding, Mapping):
        raise ValueError("v0.12 parent checkpoint binding is missing")
    ...
    if not isinstance(preflight, Mapping):
        raise ValueError("v0.12 identity preflight is missing")
    validate_final_identity_preflight_receipt(dict(preflight))
    summary = report.get("source_summary")
    if not isinstance(summary, Mapping):
        raise ValueError("v0.12 source summary is missing")
    validate_source_summary_v04(
        summary,
        expected_manifest_paths=expected_manifest_paths_v04(),
        expected_source_file_count=EXPECTED_POST_SCANNER_FILE_COUNT,
    ...
    if not isinstance(environment, Mapping):
        raise ValueError("v0.12 environment comparison is missing")
    ...
    if not isinstance(retention, Mapping) or set(retention) != {
        ...
        raise ValueError("v0.12 retention boundary changed")
    ...
    if (
        ...
        raise ValueError("v0.12 final source retention changed")
    provenance = report.get("workflow_provenance")
    if not isinstance(provenance, Mapping):
        raise ValueError("v0.12 workflow provenance is missing")
    _workflow_provenance_v12(**dict(provenance))  # type: ignore[arg-type]
    if environment.get("child_project_commit_sha") != provenance.get(
        "authorization_commit_sha"
    ):
        raise ValueError("v0.12 environment commit differs from authorization")
    ...
    if report.get("cost") != {
        ...
        raise ValueError("v0.12 cost boundary changed")
    if report.get("causal_attestation") != {
        ...
        raise ValueError("v0.12 causal attestation changed")
    if (
        ...
        raise ValueError("v0.12 final gate changed")
