"""Versioned checkpoint adapter for sealed historical source recovery v0.7.

The final pre/post-scanner layout remains 706/767 files because v0.7 recovers
...
ARTIFACT_TYPE = "sealed_historical_label_blind_source_recovery_checkpoint_v0_7"
...
@contextmanager
def _v07_contract() -> Iterator[None]:
...
def validate_authorization_envelope_v07(
...
def load_authorization_envelope_v07(path: str | Path) -> dict[str, object]:
    with _v07_contract():
...
def normalize_composite_request_budget(
    ...
    for host in ("api.massive.com", "data.sec.gov"):
        if normalized["by_host"].get(host) != PARENT_REQUEST_BUDGET["by_host"][host]:
            raise ValueError(f"v0.7 composite request budget repeated prohibited {host}")
    return normalized


def build_source_checkpoint_v07(**kwargs: object) -> dict[str, object]:
    with _v07_contract():
        return parent.build_source_checkpoint_v05(**kwargs)  # type: ignore[arg-type]


def validate_source_checkpoint_v07(
...
def build_post_scanner_checkpoint_binding_v07(
    checkpoint: Mapping[str, object],
    **kwargs: object,
) -> dict[str, object]:
    with _v07_contract():
...
def validate_post_scanner_checkpoint_binding_v07(
...
__all__ = [
    ...
    "build_source_checkpoint_v07",
    "canonical_fingerprint",
    "load_authorization_envelope_v07",
    ...
    "validate_source_checkpoint_v07",
