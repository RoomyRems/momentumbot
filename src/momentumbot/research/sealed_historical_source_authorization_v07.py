"""Authorization-only contract for sealed source recovery v0.7."""
...
from momentumbot.research.sealed_historical_source_checkpoint_v07 import (
    ...
    validate_authorization_envelope_v07,
)
...
REGISTRATION_ARTIFACT_PATHS = {
    "acquisition_validator": Path(
        "src/momentumbot/research/sealed_historical_source_acquisition_v07.py"
    ),
    "alpaca_provider": Path("src/momentumbot/providers/alpaca.py"),
    "authorization": AUTHORIZATION_PATH,
    "authorization_validator": Path(
        "src/momentumbot/research/sealed_historical_source_authorization_v07.py"
    ),
    "checkpoint_builder": Path(
        "scripts/build_sealed_historical_source_checkpoint_v07.py"
    ),
    "checkpoint_validator": Path(
        "src/momentumbot/research/sealed_historical_source_checkpoint_v07.py"
    ),
    "documentation": Path(
        "docs/research/sealed_historical_source_acquisition_v07.md"
    ),
    "failure_audit": PARENT_FAILURE_AUDIT_PATH,
    "downstream_identity_compatibility": Path(
        "src/momentumbot/historical_float_identity_v07.py"
    ),
    ...
    "news_adapter": Path("scripts/build_causal_news_enrichment_v07.py"),
    ...
    "provider_wrapper": Path("scripts/run_provider_entrypoint_v07.py"),
    "provider_transport_guard": Path("scripts/run_provider_entrypoint_v04.py"),
    "recovery_runner": Path("scripts/validate_sealed_historical_source_recovery_v07.py"),
    "recovery_validator": Path(
        "src/momentumbot/research/sealed_historical_source_recovery_v07.py"
    ),
    "request_budget": Path("src/momentumbot/providers/request_budget.py"),
    "requirements": Path("requirements-sealed-source-v04.txt"),
    "runner": Path("scripts/run_sealed_historical_source_acquisition_v07.py"),
    "scanner_adapter": Path("scripts/build_causal_scanner_snapshot_v07.py"),
...
def validate_authorization(payload: Mapping[str, object]) -> None:
    validated = validate_authorization_envelope_v07(payload)
    if validated.get("content_sha256") != AUTHORIZATION_CONTENT_SHA256:
        raise ValueError("historical source v0.7 differs from frozen hash")
    if validated.get("dispatcher_contract") != {
        ...
        raise ValueError("v0.7 dispatcher contract changed")
    if validated.get("reproducibility_environment_contract") != {
        ...
        raise ValueError("v0.7 reproducibility environment contract changed")
    ...
    if not isinstance(provider, Mapping) or provider.get("child_network_hosts") != [
        ...
        raise ValueError("v0.7 child provider network boundary changed")
    if validated.get("repair_boundary") != {
        ...
        raise ValueError("v0.7 downstream identity repair boundary changed")
...
def validate_registration_bundle() -> dict[str, object]:
    ...
    if path.is_symlink() or not path.is_file():
        raise ValueError("v0.7 registration audit must be a regular file")
    ...
    if set(audit) != expected_keys:
        raise ValueError("v0.7 registration audit fields changed")
    _validate_self_hash(
        audit,
        expected=str(audit.get("content_sha256")),
        label="v0.7 registration audit",
    )
    if (
        audit.get("schema_version") != 1
        or audit.get("artifact_type")
        != "provider_free_sealed_historical_source_recovery_v0_7_registration"
        ...
        raise ValueError("v0.7 registration identity changed")
    if audit.get("frozen_parent") != {
        ...
        raise ValueError("v0.7 registration parent changed")
    if audit.get("repair") != {
        ...
        raise ValueError("v0.7 registration repair boundary changed")
    if audit.get("authority_boundary") != {
        ...
        raise ValueError("v0.7 registration authority boundary changed")
    if audit.get("causal_attestation") != {
        ...
        raise ValueError("v0.7 registration causal attestation changed")
    artifacts = audit.get("artifacts")
    if not isinstance(artifacts, Mapping) or set(artifacts) != set(
        REGISTRATION_ARTIFACT_PATHS
    ):
        raise ValueError("v0.7 registration artifact census changed")
    for label, relative in REGISTRATION_ARTIFACT_PATHS.items():
        ...
        if (
            ...
            raise ValueError(f"v0.7 registration artifact {label} changed")
        absolute = ROOT / relative
        if absolute.is_symlink() or not absolute.is_file():
            raise ValueError(f"v0.7 registration artifact {label} is not regular")
        if entry.get("file_sha256") != _file_sha256(absolute):
            raise ValueError(f"v0.7 registration artifact {label} hash changed")
    return audit
