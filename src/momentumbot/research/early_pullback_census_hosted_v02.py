...
from momentumbot.research import early_pullback_census_hosted_v01 as previous

base = adapter.parent.parent
require, exact, seal, sha = adapter.require, adapter.exact, adapter.seal, adapter.sha
ID = "early-pullback-census-hosted-v0.2"
PARENT = "7623dadff641536bbf165846816affa427c13b1f"
PARENT_TREE = "3658e7503fc4e1accda2d1b20a7059da0b61f540"
PARENT_SHA = "1203e49170a69c5c6eb725b77aca997a61872379c69bece4bead42a7ec6a73a7"
HOSTED_PARENT_SHA = "a211bc565d25118675545b248d077050fa10e16fcb7f4f509518b85b35c915c0"
CONTRACT_PATH = f"research/strategy/{ID}.json"
EXECUTION_PATH = f"research/strategy/{ID}-execution.json"
WORKFLOW_PATH = ".github/workflows/early-pullback-census-hosted-v02.yml"
CONSUMPTION_REF = f"refs/tags/{ID}-consumed"
BASE = f"research/data-audits/{ID}"
APPROVAL_SOURCE_PATH = BASE + "/owner-authorization-source.json"
PRICING_PATH = BASE + "/public-pricing-observation.json"
LOCK = "requirements-sealed-source-v04.txt"
OWN_FILES = ("src/momentumbot/research/early_pullback_census_hosted_v02.py",
             "scripts/run_early_pullback_census_hosted_v02.py",
             "tests/test_early_pullback_census_hosted_v02.py", WORKFLOW_PATH)
...
APPROVAL_TEXT = "Approve one bounded 30-date census pass, at most 601 requests, no retries, using the documented Basic-plan reference endpoints; no subscription changes."
OWNER_MESSAGE = "I’m not sure about the massive subscription coverage but I have around $120 in free credits so I don’t mind reasonable charges. You are authorized and may proceed."


def authorization_source():
    """An operator projection of the actual message, not a subscription attestation."""
    return seal({"artifact_type": "owner_authorization_source_projection", "observed_date": "2026-09-12",
        "source": "current_user_message_in_project_conversation", "role": "user", "message": OWNER_MESSAGE,
        "user_authorized_execution_and_reasonable_charges": True,
        "subscription_coverage_owner_attested": False, "credit_balance_provider_verified": False,
        "credit_provider_identified": False, "operator_selected_maximum_incremental_cost_usd": "10.00",
        "scope": APPROVAL_TEXT, "selected_dates": list(adapter.DATES), "maximum_requests": 601,
        "retries": 0, "subscription_changes_authorized": False,
        "limitations": "message transcription and scope projection; not signed provider billing evidence"})


def pricing_observation():
    """Versioned public documentation projection; no account or balance lookup."""
    return seal({"artifact_type": "public_pricing_documentation_projection", "observed_date": "2026-09-12",
        "sources": [
            {"url": "https://massive.com/docs/rest/stocks/tickers/all-tickers",
             "facts": "ticker reference endpoint is included in all Stocks plans; Basic history is two years"},
            {"url": "https://massive.com/docs/rest/stocks/tickers/ticker-types",
             "facts": "ticker types endpoint is included in all Stocks plans"},
            {"url": "https://massive.com/pricing",
             "facts": "Stocks Basic costs USD 0 per month and allows five API calls per minute"}],
        "routes": [adapter.parent.CENSUS_ROUTE, adapter.type_request()["url"]],
        "selected_dates": list(adapter.DATES), "minimum_request_spacing_seconds": 12.5,
        "estimated_incremental_api_cost_usd": "0.00",
        "estimate_basis": "fixed API-key reference GETs fit publicly documented Basic access, history and rate limits",
        "account_entitlement_provider_verified": False, "credit_balance_provider_verified": False,
        "actual_billed_cost_usd": None, "provider_billing_cap_enforced": False,
        "metered_purchase_authorized": False, "subscription_changes_authorized": False,
        "on_denial_or_failure": "retain evidence and stop; no retry, upgrade, alternate credential or paid route",
        "limitations": "manual public documentation projection and cost inference; not an account-specific quote or billing guarantee"})
...
def registration(root):
    inherited = previous.validate_registration(root)
    require(inherited["content_sha256"] == HOSTED_PARENT_SHA, "frozen hosted parent differs")
    adapter_contract = adapter.validate_registration(root)
    require(adapter_contract["content_sha256"] == PARENT_SHA, "frozen census adapter differs")
    files = set(inherited["file_bindings"]) | set(OWN_FILES) | {adapter.CONTRACT_PATH,
        previous.CONTRACT_PATH, previous.BASE + "/hosted-ci-verification.json",
        APPROVAL_SOURCE_PATH, PRICING_PATH, LOCK}
    exact(base.frozen(root / APPROVAL_SOURCE_PATH), authorization_source(), "owner message projection differs")
    exact(base.frozen(root / PRICING_PATH), pricing_observation(), "public pricing projection differs")
    bindings = {}
    ...
    return seal({"contract_id": ID, "artifact_type": "unarmed_hosted_launcher_registration",
        "parent_commit": PARENT, "parent_tree": PARENT_TREE, "parent_registration_sha256": HOSTED_PARENT_SHA,
        "adapter_registration_sha256": PARENT_SHA,
        ...
        "entitlement_basis": "publicly documented Basic access; private account entitlement and credit balance remain unverified",
        ...
        "next_gate": "sole-file execution child of successfully tested code using the recorded owner authorization and public pricing observation",
        **adapter.BOUNDARY})
...
def validate_approval(value, now):
    ...
    observed = stamp(pricing_observation()["observed_date"] + "T00:00:00Z")
    require(observed <= now < observed + timedelta(days=7), "public pricing observation is stale or future")
    require(all(observed.date() - timedelta(days=730) <= datetime.fromisoformat(d).date() <= observed.date()
                for d in adapter.DATES), "selected dates outside documented Basic history")
    exact(value, {"approved_at": value["approved_at"], "expires_at": value["expires_at"],
        "approval_record_sha256": sha(adapter.render(authorization_source())),
        "pricing_record_sha256": sha(adapter.render(pricing_observation())),
        ...
        "maximum_authorized_incremental_cost_usd": "10.00", "estimated_incremental_api_cost_usd": "0.00",
        "provider_billing_cap_enforced": False, "metered_purchase_authorized": False,
        "subscription_changes_authorized": False}, "approval/pricing scope differs")
    return value
...
def verify_preflight(files, contract, adapter_contract, execution, env, runtime, live_ref, artifact):
    ...
    require(re.fullmatch(r"[1-9][0-9]*", env.get("CENSUS_PREFLIGHT_ARTIFACT_ID", ""))
        ...
        and artifact.get("name") == "early-pullback-census-hosted-v02-consumption-" + env["GITHUB_RUN_ID"] + "-1"
