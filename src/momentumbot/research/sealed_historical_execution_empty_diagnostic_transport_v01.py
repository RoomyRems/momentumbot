...
class DiagnosticHTTP:
    ...
    def post(self, url, data=None, params=None, basic_auth=False):
        ...
        try:
            with self.session.post(url,data=data,auth=(self.key,''),
                headers={'Accept':'application/json','User-Agent':'momentumbot-execution-empty-diagnostic-v01'},
    ...
    def stream(self, url, data, basic_auth, path):
        ...
        try:
            with self.session.post(url,data=data,auth=(self.key,''),
                headers={'Accept':'application/octet-stream','User-Agent':'momentumbot-execution-empty-diagnostic-v01'},
