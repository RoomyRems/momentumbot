"""Strict provider-free final report for sealed source recovery v0.5."""
...
SCHEMA_VERSION = 1
ARTIFACT_TYPE = "sealed_historical_source_recovery_v0_5_result"
...
def _frozen_authorization_content_sha256() -> str:
    from momentumbot.research.sealed_historical_source_authorization_v05 import (
        AUTHORIZATION_CONTENT_SHA256,
    )

    if _SHA256.fullmatch(AUTHORIZATION_CONTENT_SHA256) is None:
        raise ValueError("frozen v0.5 authorization hash is invalid")
...
def _workflow_provenance_v05(
    *,
    ...
    if repository != EXPECTED_REPOSITORY:
        raise ValueError("v0.5 repository changed")
    for label, value in (
    ...
    if not isinstance(workflow_run_id, str) or _RUN_ID.fullmatch(workflow_run_id) is None:
        raise ValueError("v0.5 workflow run ID must be a positive decimal")
    if isinstance(workflow_run_attempt, bool) or workflow_run_attempt != 1:
        raise ValueError("v0.5 acquisition is attempt 1 only")
    return {
...
def build_acquisition_report_v05(
    *,
    ...
    if (
        ...
        raise ValueError("report authority is not the frozen v0.5 child")
    checkpoint = validate_post_scanner_checkpoint_binding_v05(
    ...
    if retained > MAX_RETAINED_BYTES:
        raise ValueError("v0.5 retained-byte ceiling exceeded")
    provenance = _workflow_provenance_v05(
        repository=repository,
        authorization_commit_sha=authorization_commit_sha,
        authorization_tree_sha=authorization_tree_sha,
        dispatcher_workflow_sha=dispatcher_workflow_sha,
    ...
    if checkpoint.get("authorization") != {
        ...
        raise ValueError("v0.5 checkpoint is bound to another authorization")
    if checkpoint.get("request_budget") != budget:
        raise ValueError("v0.5 checkpoint request accounting differs from report")
    if checkpoint.get("provenance") != provenance:
        raise ValueError("v0.5 checkpoint provenance differs from report")
    if (
        ...
        raise ValueError("v0.5 checkpoint final tree differs from source summary")
    ...
    validate_acquisition_report_v05(report)
    return report


def validate_acquisition_report_v05(report: Mapping[str, object]) -> None:
    expected_keys = {
        "schema_version",
        "artifact_type",
        "authorization_id",
    ...
    if set(report) != expected_keys:
        raise ValueError("v0.5 acquisition report fields changed")
    ...
    if not isinstance(claimed, str) or _SHA256.fullmatch(claimed) is None or claimed != canonical_fingerprint(unsigned):
        raise ValueError("v0.5 acquisition report hash mismatch")
    if (
        ...
        raise ValueError("unsupported v0.5 acquisition report")
    checkpoint = report.get("source_checkpoint")
    if not isinstance(checkpoint, Mapping):
        raise ValueError("v0.5 acquisition checkpoint binding is missing")
    checkpoint = validate_post_scanner_checkpoint_binding_v05(checkpoint)
    if report.get("parent_recovery") != checkpoint.get("recovery"):
        raise ValueError("v0.5 parent recovery binding differs from checkpoint")
    if report.get("normalization_diagnostics") != checkpoint.get(
        "normalization_diagnostics"
    ):
        raise ValueError("v0.5 normalization diagnostics differ from checkpoint")
    summary = report.get("source_summary")
    if not isinstance(summary, Mapping):
        raise ValueError("v0.5 source summary is missing")
    validate_source_summary_v04(
        summary,
        expected_manifest_paths=expected_manifest_paths_v04(),
        expected_source_file_count=EXPECTED_POST_SCANNER_FILE_COUNT,
    ...
    if dict(budget) != normalized_budget or checkpoint.get("request_budget") != budget:
        raise ValueError("v0.5 composite request accounting differs")
    ...
    if not isinstance(provenance, Mapping):
        raise ValueError("v0.5 workflow provenance is missing")
    normalized_provenance = _workflow_provenance_v05(**dict(provenance))  # type: ignore[arg-type]
    ...
    if (
        ...
        raise ValueError("v0.5 report final source commitment differs")
    if report.get("cost") != {
        "incremental_provider_cost_usd": "0",
        "databento_called": False,
    }:
        raise ValueError("v0.5 cost boundary changed")
    if report.get("causal_attestation") != {
        ...
        raise ValueError("v0.5 causal attestation changed")
    if (
        ...
        raise ValueError("v0.5 final gate changed")


def load_acquisition_report_v05(path: str | Path) -> dict[str, object]:
    from momentumbot.research.sealed_historical_source_checkpoint_v01 import (
        load_json_object,
    )

    payload = load_json_object(Path(path))
    validate_acquisition_report_v05(payload)
    return payload


__all__ = [
    "ARTIFACT_TYPE",
    "build_acquisition_report_v05",
    "load_acquisition_report_v05",
    "validate_acquisition_report_v05",
]
