"""Strict provider-free final report for sealed source recovery v0.13."""
...
SCHEMA_VERSION = 1
ARTIFACT_TYPE = "sealed_historical_source_freeze_recovery_v0_13_result"
...
def validate_recovery_environment_pair_v13(
    *,
    ...
    if child != parent:
        raise ValueError("v0.13 third-party environment differs from v0.10")
    return {
        "schema_version": 1,
...
def _workflow_provenance_v13(
    *,
    ...
    if repository != EXPECTED_REPOSITORY:
        raise ValueError("v0.13 repository changed")
    for label, value in (
    ...
    if execution_mode not in {"github_actions", "local_verification", "prelaunch_verification"}:
        raise ValueError("v0.13 execution mode is invalid")
    if execution_mode == "local_verification":
        if (
            dispatcher_workflow_sha != "" or dispatcher_workflow_ref != ""
            or workflow_run_id != "" or type(workflow_run_attempt) is not int
            or workflow_run_attempt != 0
        ):
            raise ValueError("local verification must not claim GitHub execution")
    else:
        expected_ref = EXPECTED_DISPATCHER_WORKFLOW_REF
        if execution_mode == "prelaunch_verification":
            expected_ref = (
                "RoomyRems/momentumbot/.github/workflows/"
                "sealed-historical-source-recovery-v13-e2e.yml"
                "@refs/heads/phase-3-historical-snapshot"
            )
            if dispatcher_workflow_sha != authorization_commit_sha:
                raise ValueError("prelaunch verification code and workflow commits differ")
        if _GIT_SHA.fullmatch(dispatcher_workflow_sha) is None or dispatcher_workflow_ref != expected_ref:
            raise ValueError("v0.13 dispatcher workflow ref changed")
    if execution_mode != "local_verification" and _RUN_ID.fullmatch(workflow_run_id) is None:
        raise ValueError("v0.13 workflow run ID is invalid")
    if execution_mode != "local_verification" and (
        type(workflow_run_attempt) is not int or workflow_run_attempt != 1
    ):
        raise ValueError("v0.13 is attempt 1 only")
    return {
...
def _validate_environment_comparison(
    ...
    if set(value) != expected_keys:
        raise ValueError("v0.13 environment comparison fields changed")
    if (
        ...
        raise ValueError("v0.13 parent environment comparison changed")
    for field in (
        "child_environment_freeze_sha256",
        "third_party_environment_sha256",
    ):
        ...
        if not isinstance(observed, str) or re.fullmatch(r"[0-9a-f]{64}", observed) is None:
            raise ValueError(f"v0.13 environment {field} is invalid")
    ...
    if not isinstance(child_commit, str) or _GIT_SHA.fullmatch(child_commit) is None:
        raise ValueError("v0.13 child environment commit is invalid")
    return dict(value)


def build_recovery_report_v13(
    *,
    ...
    if (
        ...
        raise ValueError("report authority is not the frozen v0.13 child")
    ...
    if (
        ...
        raise ValueError("v0.13 parent checkpoint binding changed")
    validate_source_summary_v04(
        source_summary,
    ...
    if (
        ...
        raise ValueError("v0.13 retained bytes are invalid")
    if (
        ...
        raise ValueError("v0.13 final source differs from parent binding")
    provenance = _workflow_provenance_v13(
        execution_mode=execution_mode,
        repository=repository,
        authorization_commit_sha=authorization_commit_sha,
        authorization_tree_sha=authorization_tree_sha,
    ...
    if environment.get("child_project_commit_sha") != provenance.get(
        ...
        raise ValueError("v0.13 environment commit differs from authorization")
    report: dict[str, object] = {
        ...
        "source_acquisition_gate_passed": execution_mode == "github_actions",
        "next_gate": (
            "provider_free_label_blind_scanner_and_micro_runtime_freeze"
            if execution_mode == "github_actions"
            else "await_exact_manual_authorization_after_verification"
        ),
    }
...
def validate_recovery_report_v13(report: Mapping[str, object]) -> None:
    expected_keys = {
        "schema_version",
        "artifact_type",
        "authorization_id",
    ...
    if set(report) != expected_keys:
        raise ValueError("v0.13 recovery report fields changed")
    body = dict(report)
    claimed = body.pop("content_sha256", None)
    if claimed != canonical_fingerprint(body):
        raise ValueError("v0.13 recovery report hash mismatch")
    if (
        ...
        raise ValueError("unsupported v0.13 recovery report")
    ...
    if (
        ...
        raise ValueError("v0.13 parent checkpoint is missing")
    binding = parent.get("binding")
    if not isinstance(binding, Mapping):
        raise ValueError("v0.13 parent checkpoint binding is missing")
    ...
    if not isinstance(preflight, Mapping):
        raise ValueError("v0.13 identity preflight is missing")
    validate_final_identity_preflight_receipt(dict(preflight))
    summary = report.get("source_summary")
    if not isinstance(summary, Mapping):
        raise ValueError("v0.13 source summary is missing")
    validate_source_summary_v04(
        summary,
        expected_manifest_paths=expected_manifest_paths_v04(),
        expected_source_file_count=EXPECTED_POST_SCANNER_FILE_COUNT,
    ...
    if not isinstance(environment, Mapping):
        raise ValueError("v0.13 environment comparison is missing")
    ...
    if not isinstance(retention, Mapping) or set(retention) != {
        ...
        raise ValueError("v0.13 retention boundary changed")
    ...
    if (
        ...
        raise ValueError("v0.13 final source retention changed")
    provenance = report.get("workflow_provenance")
    if not isinstance(provenance, Mapping):
        raise ValueError("v0.13 workflow provenance is missing")
    _workflow_provenance_v13(**dict(provenance))  # type: ignore[arg-type]
    if environment.get("child_project_commit_sha") != provenance.get(
        "authorization_commit_sha"
    ):
        raise ValueError("v0.13 environment commit differs from authorization")
    if report.get("request_budget") != normalize_composite_request_budget(
        ...
        raise ValueError("v0.13 request accounting changed")
    if report.get("cost") != {
        "incremental_provider_cost_usd": "0",
        "provider_calls": 0,
        "databento_called": False,
    }:
        raise ValueError("v0.13 cost boundary changed")
    if report.get("causal_attestation") != {
        ...
        raise ValueError("v0.13 causal attestation changed")
    is_final = provenance.get("execution_mode") == "github_actions"
    expected_next = (
        "provider_free_label_blind_scanner_and_micro_runtime_freeze"
        if is_final else "await_exact_manual_authorization_after_verification"
    )
    if report.get("source_acquisition_gate_passed") is not is_final or report.get("next_gate") != expected_next:
        raise ValueError("v0.13 final gate changed")


def build_intermediate_checkpoint_v13(
    *,
    ...
    """Bind completed scanner bytes without claiming final replay passed."""
    checked = validate_post_scanner_checkpoint_binding_v10(binding)
    if (
        checked["checkpoint_file_sha256"] != PARENT_CHECKPOINT_FILE_SHA256
        or checked["checkpoint_content_sha256"] != PARENT_CHECKPOINT_CONTENT_SHA256
        or checked["provenance"] != PARENT_PROVENANCE
        or checked["request_budget"] != normalize_composite_request_budget(PARENT_REQUEST_BUDGET)
    ):
        raise ValueError("intermediate parent binding changed")
    preflight = validate_final_identity_preflight_receipt(dict(identity_preflight))
    environment = _validate_environment_comparison(environment_comparison)
    execution = _workflow_provenance_v13(**dict(provenance))
    if environment["child_project_commit_sha"] != execution["authorization_commit_sha"]:
        raise ValueError("intermediate environment commit changed")
    root = Path(source_root)
    inventory = inventory_source_tree(root, allow_scanner_snapshot_addition=True)
    if (
        inventory["file_count"] != EXPECTED_POST_SCANNER_FILE_COUNT
        or inventory["tree_content_sha256"] != checked["post_scanner_tree_content_sha256"]
        or sum(row["size_bytes"] for row in inventory["files"])
        != checked["post_scanner_retained_file_bytes"]
        or checked["post_scanner_retained_file_bytes"] > MAX_RETAINED_BYTES
    ):
        raise ValueError("intermediate source inventory differs from binding")
    scanner = load_json_object(root / "causal-scanner-snapshot-v0.3/manifest.json")
    scanner_body = dict(scanner)
    scanner_hash = scanner_body.pop("content_sha256", None)
    if (
        scanner_hash != canonical_fingerprint(scanner_body)
        or scanner.get("artifact_id") != "causal-scanner-snapshot-v0.3"
        or scanner.get("dates") != list(EXPECTED_DATES)
        or scanner.get("eligibility", {}).get("provider_free_replay_exact") is not True
    ):
        raise ValueError("intermediate scanner completion manifest is invalid")
    payload = {
        "schema_version": 1,
        "artifact_type": "sealed_source_v0_13_post_scanner_intermediate",
        "authorization_id": AUTHORIZATION_ID,
        "authorization_content_sha256": AUTHORIZATION_CONTENT_SHA256,
        "status": "intermediate_final_deep_replay_pending",
        "source_acquisition_gate_passed": False,
        "backtest_eligible": False,
        "selected_dates": list(EXPECTED_DATES),
        "source_inventory": inventory,
        "parent_checkpoint_binding": checked,
        "identity_preflight": preflight,
        "environment_comparison": environment,
        "workflow_provenance": execution,
        "scanner_root_content_sha256": scanner_hash,
        "provider_calls": 0,
    }
...
def validate_intermediate_checkpoint_v13(
    ...
    *,
    ...
    preflight = payload.get("identity_preflight")
    if not isinstance(preflight, Mapping):
        raise ValueError("intermediate identity receipt is missing")
    expected = build_intermediate_checkpoint_v13(
        source_root=source_root, binding=binding, identity_preflight=preflight,
        environment_comparison=environment_comparison, provenance=provenance,
    )
    if payload != expected:
        raise ValueError("intermediate checkpoint changed or claims final eligibility")
