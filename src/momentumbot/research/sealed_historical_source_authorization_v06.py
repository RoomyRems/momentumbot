"""Authorization-only contract for sealed source recovery v0.6."""
...
from momentumbot.research.sealed_historical_source_checkpoint_v06 import (
    ...
    validate_authorization_envelope_v06,
)
...
PARENT_FAILURE_AUDIT_CONTENT_SHA256 = (
    "9b7b6dffd3f87d2362baf998aa72ca7403d70fea1e49cab976efd9bdfd174a7e"
)
...
PARENT_FAILURE_AUDIT_PATH = Path(
    ...
    "sealed-historical-source-acquisition-v0.5-run-33516311649-failure-2026-09-01.json"
)
REGISTRATION_AUDIT_PATH = Path(
    "research/data-audits/"
    "sealed-historical-source-acquisition-v0.6-registration-2026-09-01.json"
)
...
REGISTRATION_ARTIFACT_PATHS = {
    ...
    "authorization_validator": Path(
        "src/momentumbot/research/sealed_historical_source_authorization_v06.py"
    ),
    "checkpoint_builder": Path(
        "scripts/build_sealed_historical_source_checkpoint_v06.py"
    ),
    "checkpoint_validator": Path(
        "src/momentumbot/research/sealed_historical_source_checkpoint_v06.py"
    ),
    "documentation": Path(
        "docs/research/sealed_historical_source_acquisition_v06.md"
    ),
    "failure_audit": PARENT_FAILURE_AUDIT_PATH,
    "float_adapter": Path("scripts/build_causal_float_enrichment_v06.py"),
    "float_identity_compatibility": Path(
    ...
    "parent_float_adapter": Path("scripts/build_causal_float_enrichment_v05.py"),
    "project_metadata": Path("pyproject.toml"),
    "provider_wrapper": Path("scripts/run_provider_entrypoint_v06.py"),
    "provider_transport_guard": Path("scripts/run_provider_entrypoint_v04.py"),
    "recovery_runner": Path("scripts/validate_sealed_historical_source_recovery_v06.py"),
    "recovery_validator": Path(
        "src/momentumbot/research/sealed_historical_source_recovery_v06.py"
    ),
    "request_budget": Path("src/momentumbot/providers/request_budget.py"),
    "requirements": Path("requirements-sealed-source-v04.txt"),
    "runner": Path("scripts/run_sealed_historical_source_acquisition_v06.py"),
...
def validate_authorization(payload: Mapping[str, object]) -> None:
    validated = validate_authorization_envelope_v06(payload)
    if validated.get("content_sha256") != AUTHORIZATION_CONTENT_SHA256:
        raise ValueError("historical source v0.6 differs from frozen hash")
    if validated.get("dispatcher_contract") != {
        ...
        raise ValueError("v0.6 dispatcher contract changed")
    if validated.get("reproducibility_environment_contract") != {
        ...
        raise ValueError("v0.6 reproducibility environment contract changed")
    ...
    if not isinstance(provider, Mapping) or provider.get("child_network_hosts") != [
        ...
        raise ValueError("v0.6 child provider network boundary changed")
    if validated.get("repair_boundary") != {
        "accepted_identity_kinds": ["composite_figi", "unique_cik_fallback"],
        "all_946_recovered_candidates_preflighted_before_consumption": True,
        "candidate_data_exception_classes": ["TypeError", "ValueError"],
        "candidate_data_exception_disposition": (
            "unknown_fail_closed_missing_measure_pair"
        ),
        "candidate_data_exception_message_persisted": False,
        "candidate_data_exception_scope": "one_symbol_date",
        "candidate_identity_values_rewritten": False,
        "identity_vocabulary_only_no_float_policy_change": True,
        ...
        raise ValueError("v0.6 identity repair boundary changed")
...
def validate_parent_bundle() -> dict[str, object]:
    ...
    if (
        ...
        != "fail_closed_identity_contract_vocabulary_mismatch"
    ):
        raise ValueError("v0.5 failure parent provenance changed")
    if not isinstance(artifacts, Mapping):
        raise ValueError("v0.5 preserved artifact evidence is missing")
    ...
    if (
        ...
        raise ValueError("v0.5 preserved artifact digest changed")
    if (
        ...
        or offline.get("all_candidates_satisfy_authoritative_identity_contract")
        is not True
        or not isinstance(accounting, Mapping)
        or accounting.get("observed_total_http_attempts") != 14_536
        ...
        or repair.get("v0_5_may_be_rerun") is not False
        or repair.get("market_membership_identity_and_discovery_may_be_reacquired")
        is not False
    ):
        raise ValueError("v0.5 recoverable checkpoint boundary changed")
    return {
        "v0_5_authorization": parent_authorization,
        "v0_5_failure_audit": failure,
    }
...
def validate_registration_bundle() -> dict[str, object]:
    ...
    if path.is_symlink() or not path.is_file():
        raise ValueError("v0.6 registration audit must be a regular file")
    ...
    if set(audit) != expected_keys:
        raise ValueError("v0.6 registration audit fields changed")
    _validate_self_hash(
        audit,
        expected=str(audit.get("content_sha256")),
        label="v0.6 registration audit",
    )
    if (
        audit.get("schema_version") != 1
        or audit.get("artifact_type")
        != "provider_free_sealed_historical_source_recovery_v0_6_registration"
        ...
        raise ValueError("v0.6 registration identity changed")
    if audit.get("frozen_parent") != {
        ...
        "v0_5_authorization_content_sha256": PARENT_AUTHORIZATION_CONTENT_SHA256,
        "v0_5_workflow_run_id": PARENT_RUN_ID,
    }:
        raise ValueError("v0.6 registration parent changed")
    if audit.get("repair") != {
        "all_946_candidate_identities_preflighted": True,
        ...
        "composite_parent_request_seed": 14_536,
        ...
        raise ValueError("v0.6 registration repair boundary changed")
    if audit.get("authority_boundary") != {
        ...
        raise ValueError("v0.6 registration authority boundary changed")
    if audit.get("causal_attestation") != {
        ...
        raise ValueError("v0.6 registration causal attestation changed")
    artifacts = audit.get("artifacts")
    if not isinstance(artifacts, Mapping) or set(artifacts) != set(
        REGISTRATION_ARTIFACT_PATHS
    ):
        raise ValueError("v0.6 registration artifact census changed")
    for label, relative in REGISTRATION_ARTIFACT_PATHS.items():
        ...
        if (
            ...
            raise ValueError(f"v0.6 registration artifact {label} changed")
        absolute = ROOT / relative
        if absolute.is_symlink() or not absolute.is_file():
            raise ValueError(f"v0.6 registration artifact {label} is not regular")
        if entry.get("file_sha256") != _file_sha256(absolute):
            raise ValueError(f"v0.6 registration artifact {label} hash changed")
    return audit
