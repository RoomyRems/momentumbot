"""Authorization-only contract for provider-free source recovery v0.12."""
...
AUTHORIZATION_CONTENT_SHA256 = (
    "a44c675ca2ad1c164eb5d868a74e779ed300580eb93ee643ce5c2a31c42c2c1a"
)
...
FAILED_EXECUTION_AUTHORIZATION_CONTENT_SHA256 = (
...
FAILED_EXECUTION_WORKFLOW_FILE_SHA256 = (
    "17575e9cb7179c818270ec3f5083d2942d52c24cd09a8de8a9c88d34a2a6cf60"
)
...
EXPECTED_DISPATCHER_WORKFLOW_REF = (
    ...
    "sealed-historical-source-acquisition-v12.yml@refs/heads/main"
)

ROOT = Path(__file__).resolve().parents[3]
AUTHORIZATION_PATH = Path(
    "research/strategy/sealed-historical-source-acquisition-v0.12.json"
)
...
FAILED_EXECUTION_AUTHORIZATION_PATH = Path(
    "research/strategy/sealed-historical-source-acquisition-v0.11.json"
)
FAILED_EXECUTION_AUDIT_PATH = Path(
    "research/data-audits/"
    "sealed-historical-source-acquisition-v0.11-run-33928334660-"
    "failure-2026-09-04.json"
)
...
FAILED_EXECUTION_WORKFLOW_PATH = Path(
    ".github/workflows/sealed-historical-source-acquisition-v11.yml"
)
REGISTRATION_AUDIT_PATH = Path(
    "research/data-audits/"
    "sealed-historical-source-acquisition-v0.12-registration-2026-09-04.json"
)
...
REGISTRATION_ARTIFACT_PATHS = {
    ...
    "authorization_validator": Path(
        "src/momentumbot/research/sealed_historical_source_authorization_v12.py"
    ),
    "documentation": Path(
        "docs/research/sealed_historical_source_acquisition_v12.md"
    ),
    "data_parent_failure_audit": PARENT_FAILURE_AUDIT_PATH,
    "failed_execution_audit": FAILED_EXECUTION_AUDIT_PATH,
    "failed_execution_authorization": FAILED_EXECUTION_AUTHORIZATION_PATH,
    "failed_execution_workflow": FAILED_EXECUTION_WORKFLOW_PATH,
    "final_identity_adapter": Path(
        "src/momentumbot/historical_float_identity_v11.py"
    ),
    "final_report": Path(
        "src/momentumbot/research/sealed_historical_source_acquisition_v12.py"
    ),
    ...
    "runner": Path("scripts/run_sealed_historical_source_acquisition_v12.py"),
    "workflow": Path(
        ".github/workflows/sealed-historical-source-acquisition-v12.yml"
    ),
    "workflow_regression_test": Path(
        "tests/test_sealed_historical_source_workflow_v12.py"
    ),
}
...
def validate_authorization(payload: Mapping[str, object]) -> None:
    ...
    if set(payload) != expected_keys:
        raise ValueError("v0.12 authorization fields changed")
    if (
        payload.get("schema_version") != SCHEMA_VERSION
        or payload.get("artifact_type")
        != "preregistered_sealed_historical_source_freeze_recovery_v0_12"
        or payload.get("authorization_id") != AUTHORIZATION_ID
        or payload.get("registered_at_date") != "2026-09-04"
    ):
        raise ValueError("v0.12 authorization identity changed")
    if payload.get("dispatcher_contract") != {
        ...
        raise ValueError("v0.12 dispatcher boundary changed")
    if payload.get("provider_free_contract") != {
        ...
        raise ValueError("v0.12 provider-free boundary changed")
    if payload.get("one_shot_contract") != {
        ...
        raise ValueError("v0.12 one-shot boundary changed")
    ...
    if not isinstance(failed, Mapping) or (
        ...
        raise ValueError("v0.12 failed-parent boundary changed")
    if payload.get("failed_execution") != {
        "authorization_commit_sha": (
            "bad5bfb048f3e4d5690038dfc4dd7c9c0968ea63"
        ),
        "authorization_content_sha256": (
            FAILED_EXECUTION_AUTHORIZATION_CONTENT_SHA256
        ),
        "authorization_id": "sealed-historical-source-acquisition-v0.11",
        "authorization_tree_sha": "e8bb6290721a93f0904012363ebdff34313ce3e3",
        "dispatcher_workflow_sha": "1a4f9f7bf483c139c4d656b2ab0a23279e8cf769",
        "failure_artifact_id": 9_957_636_441,
        "failure_artifact_zip_sha256": (
        ...
        "failed_before_checkpoint_deep_validation": True,
        "provider_calls": 0,
        "safe_failure_content_sha256": (
            "f30b6cb90b73de8f07fbb8beea360cac5791d21b049522e75320f7c1fbfa8234"
        ),
        "workflow_run_attempt": 1,
        "workflow_run_id": 33_928_334_660,
    }:
        raise ValueError("v0.12 failed-execution boundary changed")
    ...
    if not isinstance(request, Mapping) or request.get(
        ...
        raise ValueError("v0.12 request accounting changed")
...
def validate_parent_bundle() -> dict[str, object]:
    ...
    from momentumbot.research.sealed_historical_source_authorization_v11 import (
        load_authorization as load_failed_execution_authorization,
    )
    ...
    failed_execution_authorization = load_failed_execution_authorization(
        ROOT / FAILED_EXECUTION_AUTHORIZATION_PATH
    )
    if (
        failed_execution_authorization.get("content_sha256")
        != FAILED_EXECUTION_AUTHORIZATION_CONTENT_SHA256
    ):
        raise ValueError("v0.11 failed-execution authorization changed")
    failed_execution = _load_json_object(ROOT / FAILED_EXECUTION_AUDIT_PATH)
    _validate_self_hash(
        failed_execution,
        expected=FAILED_EXECUTION_AUDIT_CONTENT_SHA256,
        label="v0.11 failure audit",
    )
    if (
        failed_execution.get("conclusion")
        != "fail_closed_same_step_virtualenv_interpreter_scope_mismatch"
        or failed_execution.get("workflow", {}).get("run_id")  # type: ignore[union-attr]
        != 33_928_334_660
        or failed_execution.get("failure_artifact", {}).get("artifact_id")  # type: ignore[union-attr]
        != 9_957_636_441
    ):
        raise ValueError("v0.11 failed-execution boundary changed")
    failed_workflow = ROOT / FAILED_EXECUTION_WORKFLOW_PATH
    if (
        failed_workflow.is_symlink()
        or not failed_workflow.is_file()
        or _file_sha256(failed_workflow)
        != FAILED_EXECUTION_WORKFLOW_FILE_SHA256
    ):
        raise ValueError("v0.11 failed-execution workflow changed")
    return {
        "v0_10_authorization": parent,
        "v0_10_failure_audit": failure,
        "v0_10_provider_checkpoint_metadata": metadata,
        "v0_11_authorization": failed_execution_authorization,
        "v0_11_failure_audit": failed_execution,
    }
...
def validate_registration_bundle() -> dict[str, object]:
    ...
    if claimed != canonical_fingerprint(body):
        raise ValueError("v0.12 registration audit content hash changed")
    if set(audit) != {
        "artifact_type",
        "artifacts",
        "authority_boundary",
        ...
        raise ValueError("v0.12 registration fields changed")
    if (
        audit.get("schema_version") != 1
        or audit.get("artifact_type")
        != "provider_free_sealed_historical_source_recovery_v0_12_registration"
        ...
        raise ValueError("v0.12 registration identity changed")
    if audit.get("authority_boundary") != {
        ...
        raise ValueError("v0.12 registration authority boundary changed")
    if audit.get("causal_attestation") != {
        ...
        raise ValueError("v0.12 registration causal boundary changed")
    if audit.get("failed_execution") != {
        "authorization_content_sha256": (
            FAILED_EXECUTION_AUTHORIZATION_CONTENT_SHA256
        ),
        "failure_artifact_id": 9_957_636_441,
        ...
        raise ValueError("v0.12 registration failed-execution boundary changed")
    if audit.get("frozen_parent") != {
        ...
        "provider_checkpoint_artifact_id": 9_877_181_150,
        "provider_checkpoint_content_sha256": (
            "fef36fbcf2844f1da8510572a95c2f2978509bd2b021d2227c03c5ba5f3466f9"
        ),
        "provider_checkpoint_zip_sha256": (
            "b13bb68c5c231ba51b73c63d2a0d7e73fa78a0a837d4e35b94a55ddf5006b3b3"
        ),
        "workflow_run_id": 33_706_372_901,
    }:
        raise ValueError("v0.12 registration data-parent boundary changed")
    if audit.get("repair") != {
        ...
        "same_step_environment_comparison_uses_explicit_v0_12_interpreter": True,
        ...
        "v0_11_rerun_allowed": False,
        "workflow_only_repair": True,
    }:
        raise ValueError("v0.12 registration repair boundary changed")
    artifacts = audit.get("artifacts")
    if not isinstance(artifacts, Mapping) or set(artifacts) != set(
        REGISTRATION_ARTIFACT_PATHS
    ):
        raise ValueError("v0.12 registration artifact census changed")
    for label, relative in REGISTRATION_ARTIFACT_PATHS.items():
        ...
        if (
            ...
            raise ValueError(f"v0.12 registration artifact {label} changed")
    return audit
