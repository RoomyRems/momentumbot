...
CAUSAL_SCANNER_SNAPSHOT_V0_2_POLICY_ID = "causal-scanner-snapshot-v0.2"
CAUSAL_SCANNER_SNAPSHOT_V0_2_ARTIFACT_ID = CAUSAL_SCANNER_SNAPSHOT_V0_2_POLICY_ID
...
NORMALIZED_RANK_MINUTE_ADJUSTMENT = "split"
...
def causal_scanner_snapshot_v0_2_manifest() -> dict[str, object]:
    parent = causal_scanner_snapshot_v0_1_manifest()
    payload = {key: value for key, value in parent.items() if key != "fingerprint"}
    payload.update(
        {
            "policy_id": CAUSAL_SCANNER_SNAPSHOT_V0_2_POLICY_ID,
            "source_market_policy_id": CAUSAL_MARKET_DISCOVERY_V0_3_POLICY_ID,
            "source_market_candidate_artifact_id": (
                CAUSAL_MARKET_CANDIDATES_V0_3_ARTIFACT_ID
            ),
            "candidate_bar_rule": (
                "exact_completed_raw_candidate_bar_for_actual_price_and_volume_"
                "with_same_timestamp_split_rank_bar_for_gain"
            ),
            "candidate_rank_frame_authority_rule": (
                "split_adjusted_all_membership_frame_is_authoritative_for_gain_"
                "and_rank_raw_candidate_frame_is_authoritative_for_price_volume"
            ),
            "candidate_rank_frame_volume_match_rule": (
                "not_compared_because_split_rank_volume_is_not_a_strategy_input"
            ),
            "market_feature_rule": (
                "raw_close_and_volume_split_consistent_gain_and_exact_same_time_"
                "split_rvol"
            ),
            "supersedes_policy_id": CAUSAL_SCANNER_SNAPSHOT_POLICY_ID,
            "supersedes_policy_fingerprint": parent["fingerprint"],
        }
    )
    basis = dict(payload["rank_acquisition_basis"])
    basis["minute_adjustment"] = NORMALIZED_RANK_MINUTE_ADJUSTMENT
    payload["rank_acquisition_basis"] = basis
    return {**payload, "fingerprint": _json_fingerprint(payload)}
...
def validate_normalized_rank_candidate_coverage(
    *,
    membership_symbols: Iterable[str],
    normalized_rank_frames: Mapping[str, pd.DataFrame],
    raw_candidate_frames: Mapping[str, pd.DataFrame],
) -> None:
    """Corroborate timestamps while keeping gain and actual-price bases apart."""

    symbols = set(_validated_membership_symbols(membership_symbols))
    if set(normalized_rank_frames) - symbols or set(raw_candidate_frames) - symbols:
        raise ValueError("normalized rank/candidate frames contain nonmembers")
    for symbol, raw in raw_candidate_frames.items():
        normalized = normalized_rank_frames.get(symbol)
        if normalized is None or normalized.empty or raw.empty:
            raise ValueError(f"normalized rank/candidate bars are empty for {symbol}")
        _validate_frame(raw, label=f"raw candidate bars for {symbol}")
        _validate_frame(normalized, label=f"normalized rank bars for {symbol}")
        if not normalized.index.equals(raw.index):
            raise ValueError(
                "normalized-rank/raw-candidate timestamp coverage mismatch for "
                + symbol
            )
        if "close" not in normalized or "close" not in raw or "volume" not in raw:
            raise ValueError(f"normalized rank/candidate fields are incomplete for {symbol}")
...
def build_scanner_snapshot_rows(
    *,
    ...
    rank_minute_adjustment: str = RANK_MINUTE_ADJUSTMENT,
    ...
    if rank_minute_adjustment == RANK_MINUTE_ADJUSTMENT:
    ...
    elif rank_minute_adjustment == NORMALIZED_RANK_MINUTE_ADJUSTMENT:
        validate_normalized_rank_candidate_coverage(
            membership_symbols=symbols,
            normalized_rank_frames=rank_raw_minute_bars_by_symbol,
            raw_candidate_frames=candidate_raw_minute_bars_by_symbol,
        )
    else:
        raise ValueError("unsupported scanner rank-minute adjustment")
    ...
    for symbol, activation in activations.items():
        ...
        while decision_time < cutoff:
            ...
            if price is not None and (not math.isfinite(price) or price <= 0):
                price = None
            gain_price = price
            if rank_minute_adjustment == NORMALIZED_RANK_MINUTE_ADJUSTMENT:
                rank_bar = _exact_bar(
                    rank_raw_minute_bars_by_symbol.get(symbol, pd.DataFrame()),
                    bar_started_at=bar_started_at,
                )
                gain_price = (
                    _numeric_feature(rank_bar.get("close"))
                    if rank_bar is not None
                    else None
                )
            percent_gain = (
                (gain_price / rank_previous - 1.0) * 100.0
                if gain_price is not None
                else None
            )
            cumulative = (
                _cumulative_volume(
...
def iter_market_input_records(
    *,
    ...
    candidate_raw_minute_bars_by_symbol: Mapping[str, pd.DataFrame],
    candidate_exact_rvol_by_symbol: Mapping[str, pd.Series],
    artifact_id: str = CAUSAL_SCANNER_SNAPSHOT_ARTIFACT_ID,
...
def build_causal_scanner_snapshot_artifacts(
    *,
    ...
    source_hashes: Mapping[str, str],
    scanner_policy: dict[str, object] | None = None,
    source_market_policy: dict[str, object] | None = None,
    artifact_id: str = CAUSAL_SCANNER_SNAPSHOT_ARTIFACT_ID,
    ...
    scanner_contract = scanner_policy or causal_scanner_snapshot_v0_1_manifest()
    market_contract = source_market_policy or causal_market_discovery_v0_2_manifest()
    payload: dict[str, object] = {
        "schema_version": 1,
        "artifact_id": artifact_id,
        "trading_date": trading_date.isoformat(),
    ...
    manifest: dict[str, object] = {
        ...
        "artifact_id": artifact_id,
        "trading_date": trading_date.isoformat(),
        "scanner_policy": scanner_contract,
        "source_market_policy": market_contract,
    ...
    validate_causal_scanner_snapshot(
        ...
        expected_scanner_policy=scanner_contract,
        expected_market_policy=market_contract,
...
def validate_causal_scanner_snapshot(
    ...
    *,
    ...
    expected_scanner_policy: dict[str, object] | None = None,
    expected_market_policy: dict[str, object] | None = None,
    expected_artifact_id: str = CAUSAL_SCANNER_SNAPSHOT_ARTIFACT_ID,
) -> None:
    _validate_feature_only_profile(profile)
    ...
    scanner_contract = expected_scanner_policy or causal_scanner_snapshot_v0_1_manifest()
    market_contract = expected_market_policy or causal_market_discovery_v0_2_manifest()
    if manifest.get("scanner_policy") != scanner_contract:
        raise ValueError("causal scanner snapshot policy mismatch")
    if manifest.get("source_market_policy") != market_contract:
