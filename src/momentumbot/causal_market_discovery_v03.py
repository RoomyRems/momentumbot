...
from .historical_data_v03 import DiscoveryResult, asset_master_fingerprint
...
CAUSAL_MARKET_DISCOVERY_V0_3_POLICY_ID = "causal-market-discovery-v0.3"
...
CAUSAL_MARKET_CANDIDATES_V0_3_ARTIFACT_ID = "causal-market-candidates-v0.3"
...
class FrozenCausalMarketDiscoveryPolicy:
    ...
    gain_normalization_rule: str | None = None
...
def causal_market_discovery_v0_3_policy(
) -> FrozenCausalMarketDiscoveryPolicy:
    return FrozenCausalMarketDiscoveryPolicy(
        policy_id=CAUSAL_MARKET_DISCOVERY_V0_3_POLICY_ID,
        status=CAUSAL_MARKET_DISCOVERY_POLICY_STATUS,
        source_universe_policy_id=IDENTITY_RESOLVED_UNIVERSE_POLICY_ID,
        daily_history_calendar_days=120,
        exact_rvol_prior_sessions=50,
        ...
        coarse_rvol_bar_size="15Min",
        acquisition_prefilter_rule=(
            "split_target_high_over_split_previous_close_for_gain_superset_"
            "and_raw_target_range_for_price_superset_only"
        ),
        strategy_input_rule=(
            "raw_target_1min_price_and_volume_split_target_1min_gain_and_"
            "exact_same_time_split_rvol_at_decision_availability"
        ),
        minute_bar_timestamp_semantics="alpaca_1min_timestamp_is_bar_start",
        decision_availability_rule=(
            "bar_close_and_same_time_rvol_available_at_bar_start_plus_1_minute"
        ...
        supersedes_policy_id=CAUSAL_MARKET_DISCOVERY_POLICY_ID,
        supersedes_policy_fingerprint=causal_market_discovery_v0_2_manifest()[
            "fingerprint"
        ],
        gain_normalization_rule=(
            "percent_gain_uses_split_target_close_over_split_previous_close_"
            "while_price_threshold_uses_raw_target_close"
        ),
    )


def causal_market_discovery_v0_3_manifest() -> dict[str, object]:
    policy = causal_market_discovery_v0_3_policy()
    return {**policy.payload(), "fingerprint": policy.fingerprint}
...
def build_market_candidate_payload(
    *,
    ...
    discovery_policy: dict[str, object] | None = None,
    candidate_artifact_id: str = CAUSAL_MARKET_CANDIDATES_ARTIFACT_ID,
) -> dict[str, object]:
    policy = discovery_policy or causal_market_discovery_v0_2_manifest()
...
def load_market_candidate_payload(
    ...
    policy_id = manifest.get("artifact_id")
    if policy_id not in {
    ...
    expected_policy = (
        ...
        if policy_id == CAUSAL_MARKET_DISCOVERY_V0_3_POLICY_ID
        else causal_market_discovery_v0_2_manifest()
    )
    if manifest.get("discovery_policy") != expected_policy:
    ...
    if payload.get("artifact_id") != expected_candidate_id:
...
def build_causal_market_discovery_manifest(
    *,
    ...
    if policy["policy_id"] == CAUSAL_MARKET_DISCOVERY_V0_3_POLICY_ID:
        acquisition["target_gain_adjustment"] = "split"
        acquisition["gain_and_price_series_separated"] = True
