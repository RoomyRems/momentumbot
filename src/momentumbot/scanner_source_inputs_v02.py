...
from .causal_scanner_snapshot_v02 import (
    CAUSAL_SCANNER_SNAPSHOT_POLICY_ID,
    CAUSAL_SCANNER_SNAPSHOT_V0_2_ARTIFACT_ID,
    CAUSAL_SCANNER_SNAPSHOT_V0_2_POLICY_ID,
...
def write_scanner_source_input_bundle(
    ...
    *,
    ...
    scanner_policy: Mapping[str, object] | None = None,
    scanner_artifact_id: str = CAUSAL_SCANNER_SNAPSHOT_POLICY_ID,
    ...
    policy = dict(scanner_policy or causal_scanner_snapshot_v0_1_manifest())
    ...
    with record_path.open("wb") as raw:
        with gzip.GzipFile(
            ...
            for kind, value in iter_market_input_records(
                ...
                artifact_id=scanner_artifact_id,
            ):
    ...
    manifest: dict[str, object] = {
        ...
        "format": FORMAT_ID,
        "scanner_policy_id": policy["policy_id"],
...
def validate_scanner_source_input_manifest(
    ...
    policy_id = manifest.get("scanner_policy_id")
    if policy_id not in {
        CAUSAL_SCANNER_SNAPSHOT_POLICY_ID,
        CAUSAL_SCANNER_SNAPSHOT_V0_2_POLICY_ID,
    }:
    ...
    policy = (
        causal_scanner_snapshot_v0_2_manifest()
        if policy_id == CAUSAL_SCANNER_SNAPSHOT_V0_2_POLICY_ID
        else causal_scanner_snapshot_v0_1_manifest()
    )
    if manifest.get("scanner_policy_fingerprint") != policy["fingerprint"]:
    ...
    expected_acquisition = {
        ...
        "minute_timeframe": RANK_MINUTE_TIMEFRAME,
        "minute_adjustment": policy["rank_acquisition_basis"]["minute_adjustment"],
        "asof_rule": RANK_ACQUISITION_ASOF_RULE,
    }
...
def load_scanner_source_input_bundle(
    ...
    normalized_rank = (
        manifest.get("acquisition_basis", {}).get("minute_adjustment") == "split"
    )
    for symbol in candidates:
        ...
        if not normalized_rank and not rank_close.equals(candidate_close):
            raise ValueError(
    ...
    logical_sha = market_inputs_fingerprint(
        ...
        artifact_id=(
            CAUSAL_SCANNER_SNAPSHOT_V0_2_ARTIFACT_ID
            if normalized_rank
            else CAUSAL_SCANNER_SNAPSHOT_POLICY_ID
        ),
    )
